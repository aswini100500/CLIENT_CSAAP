...
const CompactDetailRow = ({ label, value, icon: IconComponent }) => (
  <div className="flex items-center justify-between gap-4 py-2 border-b border-slate-100 last:border-0">
    <div className="flex items-center gap-2">
      {IconComponent && (
        <IconComponent className="size-4 text-(--text-faint) shrink-0" />
      )}
...
const ProjectDetailsModal = ({ project, onClose }) => {
  ...
  const isDuplex =
    normalizedType.includes("duplex") ||
  ...
  if (isPlotting) {
    ...
    tabs = [{ id: "details", label: "Specifications" }];
  }
          ...
          <div className="overflow-x-auto rounded-2xl border border-(--border-soft) bg-white">
            <table className="min-w-full divide-y divide-slate-100">
              <thead className="bg-slate-50">
                <tr>
                  <th className="text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft) text-left px-4 py-3">
                    Plot No
                  </th>
                  <th className="text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft) text-left px-4 py-3">
                    Area (Sqft)
              ...
              <tbody className="divide-y divide-slate-100 text-[13px] text-(--text-body)">
                {data.length > 0 ? (
                  data.map((plot, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-0.5 rounded text-[11px] font-semibold inline-block capitalize ${getStatusBadge(plot.status)}`}
                        >
                  ...
                  <tr>
                    <td
                      colSpan={4}
                      className="px-4 py-8 text-center text-(--text-faint) font-medium"
                    >
                  ...
                  data.map((block, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-medium">
                        {block.units_per_floor}
                      </td>
                      <td className="px-4 py-3 font-bold text-(--text-strong)">
                        {block.total_units}
                      </td>
                    </tr>
                  ))
              ...
              data.map((app, index) => (
                ...
                >
                  <div className="min-w-0 pr-4">
                    <h5 className="text-[13.5px] font-bold text-(--text-strong) truncate">
                      {app.authority}
                    </h5>
                    <p className="text-[11px] text-(--text-faint) mt-0.5 font-medium">
                      {app.date
                        ? `Verified on ${new Date(app.date).toLocaleDateString()}`
                ...
                {data.length > 0 ? (
                  data.map((unit, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-0.5 rounded text-[11px] font-semibold inline-block capitalize ${getStatusBadge(unit.status)}`}
                        >
                  ...
                  <tr>
                    <td
                      colSpan={isCommercial || isDuplex ? 5 : 4}
                      className="px-4 py-8 text-center text-(--text-faint) font-medium"
                    >
        ...
        return (
          <div className="space-y-4">
            <div className="app-panel p-4 bg-white border border-(--border-soft)">
              <h5 className="text-[12px] font-bold text-(--text-soft) uppercase tracking-wider mb-3">
                  ...
                  facilityKeys.map((key) => {
                    ...
                    return (
                      <div key={key} className="flex items-center gap-2 py-1">
                        ...
                        >
                          <Check
                            className={`size-3 ${available ? "opacity-100" : "opacity-30"}`}
                          />
                        </div>
                        <span
                          className={`text-[12.5px] font-medium capitalize ${available ? "text-(--text-body)" : "text-(--text-faint) line-through"}`}
                        >
                          {key.replace(/_/g, " ")}
                        </span>
                      </div>
            ...
            <div className="app-panel p-4 bg-white border border-(--border-soft)">
              <h5 className="text-[12px] font-bold text-(--text-soft) uppercase tracking-wider mb-2.5">
                Premium Additions
              </h5>
              {customFacilities.length > 0 ? (
                ...
                <p className="text-[12.5px] text-(--text-faint) italic">
                  No auxiliary premium options configured.
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex gap-2.5 items-start">
              ...
              <div>
                ...
                <p className="text-[12.5px] text-(--text-soft) mt-1 leading-relaxed">
                  This project contains customized setups. Please check standard
                  reports or contact administrative desks for custom design
                  layout sheets.
                </p>
              </div>
            </div>
        ...
        <div className="px-5 py-4 border-b border-(--border-soft) flex justify-between items-start bg-white">
          <div className="flex items-start gap-3.5 min-w-0 pr-4">
            <div className="size-11 rounded-2xl flex items-center justify-center bg-(--brand-soft) border border-(--border-soft) shrink-0">
              ...
              {!isPlotting && !isApartment && !isDuplex && !isCommercial && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
              ...
              <div>
                {isCommercial && project.commercial_sub_type && (
                  <CompactDetailRow
                    label="Category"
                    value={project.commercial_sub_type}
                    icon={Info}
                  />
                )}
