...
import { formatStatus, getStatusColor, normalizeUnits, isFinishedUnit } from "./leadUtils";
...
const ViewPaymentSlabsModal = ({
    ...
    try {
      ...
      if (unitIsFinished) {
        ...
        try {
          ...
          stagePassings = passings.filter(p => Number(p.project_setup_id) === Number(matchedSetup.id));
        } catch (err) {
      ...
      try {
        const planRes = await accountingApi.get(`/api/v1/project-payment`, {
          params: { lead_id: lead.id, company_id: user?.company_id || lead?.company_id }
        });
      ...
      if (isMounted.current) {
        if (existingPlan) {
          ...
          const mergedStages = data.map((s, idx) => {
            ...
            const constructionStatus = matchedPassing ? (matchedPassing.status || "planned") : (s.status || "planned");
          ...
          if (!unitIsFinished && mergedStages.length > 0) {
            ...
            const isOngoing = (s) => {
              const cs = (s.construction_status || "").toLowerCase().trim();
              return cs === "ongoing" || cs === "in_progress" || cs === "in progress";
            };
            ...
            const firstNonCompletedIdx = mergedStages.findIndex(s => !isCompleted(s));
  ...
  const totalPaid = stages.reduce((sum, s) => sum + (s.paid_amount || 0), 0) + (bookingSlab ? bookingSlab.paid_amount : 0);
      ...
      <div className="app-modal w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-5 overflow-y-auto custom-scrollbar flex-1 space-y-5 bg-[#fcfdfd]">
          {loading ? (
            <div className="p-16 text-center">
              ...
              <p className="text-[13.5px] font-semibold text-(--text-soft)">Loading payment plans...</p>
            </div>
          ) : !hasProject ? (
            <div className="p-16 text-center max-w-md mx-auto space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[12.5px] text-(--text-soft)">
                  No project assigned to this lead. Please select a project first.
                </p>
              </div>
            </div>
          ) : !hasSetup ? (
            <div className="p-16 text-center max-w-md mx-auto space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[12.5px] text-(--text-soft)">
                  No project setup has been defined for this project. Setup a template first.
                </p>
              </div>
            </div>
          ) : error ? (
            <div className="p-16 text-center max-w-md mx-auto space-y-4">
              ...
              <div className="space-y-1.5">
                <h4 className="text-[15px] font-extrabold text-(--text-strong)">Plan Not Found</h4>
            ...
            <>
              {paymentPlan?.status === 'cancelled' && (
                <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-[13px] font-bold text-red-800">Booking Cancelled & Settled</h4>
                    <p className="text-[12px] text-red-650 mt-0.5">
                      This customer's booking has been officially cancelled. All payment collections are terminated and the unit is released.
                    </p>
                  </div>
                </div>
              )}
              ...
              {paymentPlan?.status === 'pending_cancellation' && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-[13px] font-bold text-amber-800">Cancellation Request Pending</h4>
                    <p className="text-[12px] text-amber-650 mt-0.5">
                      A request to cancel this booking has been initiated and is currently in the verification queue.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white p-4 rounded-2xl border border-(--border-soft) shadow-2xs flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] font-bold text-(--text-faint) uppercase tracking-wider block">Contract Value</span>
                ...
                <div className="bg-white p-4 rounded-2xl border border-(--border-soft) shadow-2xs flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] font-bold text-(--text-faint) uppercase tracking-wider block">Total Collected</span>
              ...
              {totalDealValue > 0 && (
                <div className="bg-white p-4 rounded-2xl border border-(--border-soft) shadow-2xs space-y-2">
                  <div className="flex items-center justify-between text-[12px] font-bold text-(--text-soft)">
                    ...
                    <span className="text-emerald-600 font-extrabold">{paidPct.toFixed(1)}% Collected</span>
                  </div>
              ...
              {bookingSlab && (
                <div className="app-panel p-4 bg-sky-50/40 border border-sky-100/70 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h4 className="text-[13.5px] font-bold text-sky-900">{bookingSlab.name}</h4>
                      <p className="text-[11px] text-sky-700/70 font-semibold mt-0.5">Initial advance confirmation payment</p>
                    </div>
                  </div>
                  
                  <div className="flex flex-wrap items-center gap-6 shrink-0 text-right sm:text-right">
                    <div>
                      <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Booking Value</span>
                      <span className="text-[13px] font-bold text-(--text-strong)">
                        {formatINR(bookingSlab.allocated_amount)} <span className="text-slate-400 font-semibold text-[11px]">({bookingSlab.ratio_percentage.toFixed(1)}%)</span>
                      </span>
                    </div>
                    
                    <div>
                      <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Paid Amount</span>
                      <span className={`text-[13px] font-bold ${bookingSlab.paid_amount > 0 ? "text-emerald-600" : "text-(--text-faint)"}`}>
                        {formatINR(bookingSlab.paid_amount)}
                      </span>
                    </div>

                    <div>
                      <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Remaining</span>
                      <span className={`text-[13px] font-bold ${bookingSlab.allocated_amount - bookingSlab.paid_amount > 0 ? "text-slate-700" : "text-slate-400"}`}>
                        {formatINR(bookingSlab.allocated_amount - bookingSlab.paid_amount)}
                      </span>
                    </div>

                    <div>
                      <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Status</span>
                      <span className={`px-2 py-0.5 rounded text-[11px] font-semibold border ${
                        bookingSlab.status === "paid" ? "text-emerald-700 bg-emerald-50 border-emerald-200" :
                        bookingSlab.status === "partial" ? "text-amber-700 bg-amber-50 border-amber-200" :
                        "text-slate-500 bg-slate-50 border-slate-200/60"
                      } inline-block mt-0.5`}>
                        {bookingSlab.status === "paid" ? "Fully Paid" : bookingSlab.status === "partial" ? "Partially Paid" : "Unpaid"}
                      </span>
                    </div>
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-6 shrink-0 text-right sm:text-right">
                          <div>
                            ...
                            <span className="text-[13px] font-bold text-(--text-strong)">
                              {formatINR(amtNum)} <span className="text-slate-400 font-semibold text-[11px]">({pctVal.toFixed(1)}%)</span>
                            </span>
                          </div>
                          
                          <div>
                            <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Paid Amount</span>
                            <span className={`text-[13px] font-bold ${stage.paid_amount > 0 ? "text-emerald-600" : "text-(--text-faint)"}`}>
                          ...
                          <div>
                            <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Remaining</span>
                            <span className={`text-[13px] font-bold ${amtNum - stage.paid_amount > 0 ? "text-slate-700" : "text-slate-400"}`}>
                          ...
                          <div>
                            <span className="text-[10px] font-bold text-(--text-faint) uppercase block tracking-wider">Status</span>
                            <span className={`px-2 py-0.5 rounded text-[11px] font-semibold border ${badgeStyle} inline-block mt-0.5`}>
                              {statusLabel}
                            </span>
                          </div>
                        </div>
        ...
        <div className="px-5 py-3 border-t border-(--border-soft) flex justify-between items-center bg-white">
          ...
          <div className="flex items-center gap-3">
            ...
            {!loading && hasProject && hasSetup && (!error || error === "No active payment slab found for this lead.") && paymentPlan?.status !== 'cancelled' && paymentPlan?.status !== 'pending_cancellation' && (
              <button
                onClick={onEdit}
...
const mockProjectSetupResponse = {
  "2": {
    ...
    "data": {
      ...
      "stages": JSON.stringify([
        { "id": 1, "name": "Stage 1", "description": "Excavation & Shoring", "start": "2026-06-01", "end": "2026-06-02", "status": "planned", "sub_stages": [] },
        { "id": 2, "name": "Stage 2", "description": "Plinth Foundation", "start": "", "end": "", "status": "planned", "sub_stages": [] },
        { "id": 3, "name": "Stage 3", "description": "RCC Slab Castings", "start": "", "end": "", "status": "planned", "sub_stages": [] },
        { "id": 4, "name": "Stage 4", "description": "Brickwork & Finishing", "start": "", "end": "", "status": "planned", "sub_stages": [] }
      ]),
      "units_data": "[]",
      "status": "draft",
  ...
  "default": {
    ...
    "data": {
      ...
      "end_date": "2026-06-13T00:00:00.000Z",
      "stages": JSON.stringify([
        { "id": 1, "name": "Booking / Token Confirmation", "description": "Token Advance confirmation", "start": "2026-06-01", "end": "2026-06-02", "status": "planned" },
        { "id": 2, "name": "Agreement Execution", "description": "Legal contract registry", "start": "", "end": "", "status": "planned" },
        { "id": 3, "name": "Foundation Plinth Complete", "description": "Excavation and foundation work", "start": "", "end": "", "status": "planned" },
        { "id": 4, "name": "RCC Superstructure Casting", "description": "RCC framing slabs", "start": "", "end": "", "status": "planned" },
        { "id": 5, "name": "Brickwork & Plastering Work", "description": "Internal and external blockworks", "start": "", "end": "", "status": "planned" },
        { "id": 6, "name": "Final Handover & Key Registry", "description": "Key hovers to customers", "start": "", "end": "", "status": "planned" }
      ]),
      "units_data": "[]",
      "status": "draft",
      "created_at": "2026-06-01T06:39:36.000Z",
...
const fetchProjectStages = async (projectId, projectType = "apartment", token) => {
  ...
  try {
    ...
    if (response.data?.success && response.data?.data) {
      ...
      const parsedStages = typeof setupData.stages === "string"
        ? JSON.parse(setupData.stages)
        : (setupData.stages || []);

      return parsedStages.map((stage, idx) => {
        const id = stage.id || stage.name?.replace(/\s+/g, "_").toLowerCase() || `stage_${idx}`;
        return {
          id,
    ...
    console.warn("Failed fetching project stages from API, resolving using structural mock setup:", err.message);
  }
