...
const CreatePaymentSlabModal = ({
        ...
        if (active) {
          ...
          if (existingPlan) {
            ...
            const planStages = existingPlan.slabs.map((matchedSlab, idx) => {
              ...
              initialAmounts[matchedId] =
                matchedSlab.allocated_amount.toString();
              initialPercentages[matchedId] =
    ...
    if (count > 0) {
      ...
      stages.forEach((stage, idx) => {
        if (idx === count - 1) {
          ...
          newAmounts[stage.id] =
            Math.round((dealValueNum - sumAmts) * 100) / 100;
        } else {
      ...
      if (
        ...
        stages.forEach((stage) => {
          ...
          if (pctVal !== "") {
            ...
            updatedAmts[stage.id] = Math.round(
  ...
  const handleDeleteMilestone = (stageId) => {
    ...
    if (
      targetStage &&
      (targetStage.paid_amount > 0 ||
        targetStage.status === "paid" ||
        targetStage.status === "partial")
    ) {
      setValidationError(
        "Cannot delete a milestone that has payments recorded.",
      );
      return;
    }
  ...
  const handleDueDateChange = (stageId, dateVal) => {
    setStages((prev) =>
      prev.map((s) =>
        s.id === stageId ? { ...s, due_date: dateVal || null } : s,
      ),
    );
  };
                            ...
                            {isEditMode &&
                              ...
                              !isPartial && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleDeleteMilestone(stage.id)
                                  }
