...
const LeadDetailsModal = ({
  ...
  const { data: brokerOptions = [] } = useQuery({
    ...
    enabled: !!token && lead?.source?.toUpperCase() === "BROKER" && !!lead?.broker_id,
  });

  const selectedBroker = brokerOptions.find(
    (b) => String(b.id) === String(lead.broker_id)
  );
  const brokerName = selectedBroker ? selectedBroker.name : (lead.broker_name || lead.broker_id);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-(--bg-subtle)">
              ...
              <div className="px-4 py-1 divide-y divide-(--bg-subtle)">
                <div className="flex items-center gap-2.5 py-2 min-w-0">
                  ...
                  <span className="text-[12px] text-(--text-faint) shrink-0">Project</span>
                  <div className="ml-auto text-right flex items-center gap-1.5 min-w-0">
                    {isLoadingProject ? (
                      <span className="text-[12px] text-(--text-faint) animate-pulse font-medium">Loading...</span>
                    ) : projectDetails ? (
                      <>
                        <span className="text-[13px] font-bold text-(--text-strong) truncate max-w-30" title={projectDetails.name}>
                          {projectDetails.name}
                        </span>
                        <button
                ...
                <DetailRow icon={Info} label="Source" value={formatSource(lead.source)} />
                {lead.source?.toUpperCase() === "BROKER" && (
                  <>
                    <DetailRow icon={User} label="Broker Name" value={brokerName || "NA"} iconColor="text-violet-500" />
                    <DetailRow icon={Hash} label="Broker Commission" value={lead.commission ? `${lead.commission}%` : "NA"} iconColor="text-amber-500" />
                  </>
                )}
                <DetailRow
                  icon={CalendarClock}
