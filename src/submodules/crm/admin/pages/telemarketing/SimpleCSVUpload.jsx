...
import api from "../../../api";
import { AlertCircle, CheckCircle2, Download, FileText, Upload, User } from 'lucide-react';
...
const HEADER_ALIASES = {
  ...
  phone: ['phone', 'phone number', 'mobile', 'mobile number', 'contact no', 'contact number'],
...
const buildHeaderMap = (headers) => {
  ...
  return Object.entries(HEADER_ALIASES).reduce((accumulator, [field, aliases]) => {
...
const SimpleCSVUpload = () => {
  ...
  const invalidRowsForDownload = useMemo(() => {
    ...
    if (validationErrors.length && previewRows.length) {
      return validationErrors.map((row) => {
        const previewRow = previewRows.find((item) => item.rowNumber === row.rowNumber);
        return {
          ...(previewRow || {}),
          row_number: row.rowNumber,
          error_reason: Array.isArray(row.issues) ? row.issues.join('; ') : row.reason,
        };
      });
    }
  ...
  const handleDownloadTemplate = () => {
    ...
    const sampleRows = [
      ['Amit Sharma', '9876543210', 'amit@example.com', 'Website', 'Mumbai', 'Aarav Patel'],
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const hasName = headerMap.name || (headerMap.first_name && headerMap.last_name);
      ...
      setUploadMessage(issues.length > 0 ? `${issues.length} row(s) need attention before import.` : '');
    } catch (error) {
      setSelectedFile(null);
      setPreviewRows([]);
      setUploadStatus('error');
  ...
  const handleUpload = async () => {
    if (!authToken || !authUser?.company_id || !authUser?.slug) {
      setUploadStatus('error');
      setUploadMessage('Login context is missing. Please sign in again before importing leads.');
      return;
    }
    ...
    if (selectedFile.name.toLowerCase().endsWith('.csv') && validationErrors.length > 0) {
      setUploadStatus('error');
    ...
    try {
      ...
      setUploadMessage(response.data?.message || 'Lead import completed successfully.');
      setSelectedFile(null);
      setPreviewRows([]);
      setCurrentPage(1);
      ...
      setUploadStatus('error');
      setUploadMessage(
        error.response?.data?.message || 'Lead import failed. Please try again.',
      );
  ...
  const handleDownloadInvalidRows = () => {
    ...
    downloadBlob(csvContent, 'invalid-lead-rows.csv', 'text/csv;charset=utf-8;');
  };
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="app-panel p-4">
            ...
            <label className={`flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-(--border-soft) bg-(--bg-subtle) px-6 py-10 text-center transition-all duration-200 hover:border-(--brand) hover:bg-white ${!canUpload ? 'pointer-events-none opacity-50' : ''}`}>
              <div className="p-2 bg-(--brand-soft) rounded-xl border border-(--border-soft) mb-3">
                <Upload className="size-4 text-(--brand)" />
              </div>
              <span className="text-[14px] font-medium text-(--text-strong)">Choose file</span>
              <span className="mt-1 text-[12px] text-(--text-soft)">CSV, XLS, or XLSX up to 20 MB</span>
              <input
                ref={inputRef}
                type="file"
                disabled={!canUpload}
            ...
            <div className="mt-4 flex items-center justify-between rounded-2xl border border-(--border-soft) bg-(--bg-subtle)/70 px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[12px] text-(--text-soft)">
                    {selectedFile ? formatFileSize(selectedFile.size) : 'Upload a file to begin import'}
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                disabled={isUploading || !selectedFile || validationErrors.length > 0 || !canUpload}
                className={`px-3 py-1.5 rounded-md active:scale-[0.98] transition-all duration-200 flex items-center text-[13px] font-medium ${
                  isUploading || !selectedFile || validationErrors.length > 0 || !canUpload
                    ...
                    <div className="space-y-1 text-[12px]">
                      {validationErrors.slice(0, 8).map((row) => (
                        <div key={row.rowNumber}>
                          Row {row.rowNumber}: {Array.isArray(row.issues) ? row.issues.join(', ') : row.reason}
                        </div>
                      ))}
                    </div>
                  )}
                  {(uploadStatus === 'error' || uploadStatus === 'warning') && invalidRowsForDownload.length > 0 && (
                    <div>
                      <button
                        type="button"
            ...
            <div className="app-panel p-4 h-full">
              ...
              <div className="mt-4 space-y-3">
                <div className="rounded-2xl border border-(--border-soft) bg-(--bg-subtle)/70 p-3">
                  <div className="text-[13px] font-medium text-(--text-strong)">Supported files</div>
                ...
                <div className="rounded-2xl border border-(--border-soft) bg-(--bg-subtle)/70 p-3">
                  <div className="text-[13px] font-medium text-(--text-strong)">Use the template</div>
                  <div className="mt-1 text-[12px] text-(--text-soft)">
                    Download the template if you want the expected column names and sample values.
                  </div>
                </div>
            ...
            {previewRows.length > 0 && (
              <div className="flex flex-col sm:flex-row justify-between items-center px-4 py-3 app-section-bar border-t border-(--border-soft)">
                <div className="text-[13px] text-(--text-soft) font-medium mb-3 sm:mb-0">
                  Showing {paginatedPreviewRows.length} of {previewRows.length} entries
                </div>
