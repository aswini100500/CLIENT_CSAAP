    ...
    const fetchData = async () => {
      ...
      try {
        const [leadsRes, customersRes] = await Promise.all([
          ...
          api.get(
            `/api/brokers/${broker.id}/customers?company_id=${companyId}`,
          ),
        ]);
                      ...
                      <table className="min-w-full text-left">
                        <thead>
                          <tr className="border-b border-(--border-soft)">
                            <th className="px-4 py-2.5 text-left text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft) bg-slate-50/50">
                              Lead Info
                            </th>
                            <th className="px-4 py-2.5 text-left text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft) bg-slate-50/50">
                              Commission
                            ...
                            <td className="px-6 py-4 text-xs font-bold text-slate-700">
                              ...
                              {cust.total_deal_value > 0 && (
                                <span className="text-[11px] text-slate-400 font-medium ml-1">
                                  (
                                  {formatCurrency(
                                    (cust.total_deal_value * cust.commission) /
                                      100,
                                  )}
                                  )
                                </span>
