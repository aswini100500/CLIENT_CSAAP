...
const LeaveManagement = () => {
  ...
  const fetchLeaves = async () => {
    try {
      const balanceRes = await axios.get(
        `${import.meta.env.VITE_HRMS_BASE_URL}/api/leaves/getRemainingLeave/${slug}/${emp_id}`,
      );
      ...
      const requestsRes = await axios.get(
        `${import.meta.env.VITE_HRMS_BASE_URL}/api/leaves/${slug}/${emp_id}`,
      );
      ...
      if (balanceData) {
        if (source === "policy") {
          totalVal =
            parseFloat(balanceData.yearly_CL || 0) +
          ...
          ["CL", "EL", "ML"].forEach((t) => {
            types[t].monthlyCredit = parseFloat(
              balanceData[`monthly_${t}`] || 0,
            );
            types[t].monthlyRemaining = parseFloat(
              balanceData[`monthly_${t}`] || 0,
            );
            types[t].totalAvailable = parseFloat(
              balanceData[`monthly_${t}`] || 0,
            );
            types[t].yearlyRemaining = parseFloat(
              balanceData[`yearly_${t}`] || 0,
            );
          });
        } else {
          ["CL", "EL", "ML"].forEach((t) => {
            const cf = parseFloat(balanceData[`carried_forward_${t}`] || 0);
            const monthlyRem = parseFloat(
              balanceData[`monthly_remaining_${t}`] || 0,
            );
            const monthlyCredit = parseFloat(
              balanceData[`policy_monthly_${t}`] || 0,
            );
            const yearlyUsed = parseFloat(balanceData[`yearly_used_${t}`] || 0);
            const yearlyRem = parseFloat(
              balanceData[`yearly_remaining_${t}`] || 0,
            );
          ...
          totalVal =
            parseFloat(balanceData.policy_yearly_CL || 0) +
            parseFloat(balanceData.policy_yearly_EL || 0) +
            parseFloat(balanceData.policy_yearly_ML || 0);
          takenVal =
            types.CL.yearlyUsed + types.EL.yearlyUsed + types.ML.yearlyUsed;
          remainingVal =
            types.CL.totalAvailable +
            types.EL.totalAvailable +
            types.ML.totalAvailable;
        }
      }

      const pendingCount = formattedLeaves.filter(
        (l) => l.status === "Pending",
      ).length;
      const approvedCount = formattedLeaves.filter(
        (l) => l.status === "Approved",
      ).length;
      const rejectedCount = formattedLeaves.filter(
        (l) => l.status === "Rejected",
      ).length;
  ...
  const leaveAllowedOptions = ["Full Day", "Half Day"];
  ...
  const validateForm = (data, isEdit = false) => {
    ...
    if (!data.leaveAllowed)
      newErrors.leaveAllowed = "Please select leave allowed";
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${API_BASE}/api/leaves/${selectedLeave.id}`,
        {
          ...editFormData,
          employee_id: selectedLeave.employee_id || user.company_id,
          company_id:
            selectedLeave.company_id || user.company_id || user.company_id,
        },
      );
      ...
      if (
        ...
        (res.data.id || res.data.employee_id)
      ) {
    ...
    try {
      ...
      try {
        ...
        if (err.response?.status === 404) {
          try {
            await axios.delete(
              `${API_BASE}/api/leaves/delete/${selectedLeave.id}`,
            );
          } catch (err2) {
            try {
              await axios.post(`${API_BASE}/api/leaves/delete`, {
                id: selectedLeave.id,
              });
            } catch (err3) {
              throw err3 || err2 || err;
      ...
      setLeaveRequests(
        leaveRequests.filter((leave) => leave.id !== selectedLeave.id),
      );
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900 mt-2">
                      {Number(leaveSummary.totalAnnualLeaves).toFixed(1)}
                    </p>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-3 border border-emerald-200">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900 mt-2">
                      {Number(leaveSummary.leavesTaken).toFixed(1)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-3 border border-amber-200">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900 mt-2">
                      {Number(leaveSummary.remainingLeaves).toFixed(1)}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 text-[11px] text-gray-500">
                      <span className="text-green-600">
                        ✓ {leaveSummary.approvedLeaves} approved
                      </span>
                      <span className="text-red-600">
                        ✗ {leaveSummary.rejectedLeaves} rejected
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider border-b">
                        Leave Type
                      </th>
                      <th className="px-4 py-3 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider border-b">
                        Carry Forward (Prev)
                      </th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    ...
                    <tr className="bg-gray-50 font-semibold">
                      ...
                      <td className="px-4 py-3 text-center text-sm text-amber-700">
                        {(
                          leaveSummary.types.CL.carryForward +
                          leaveSummary.types.EL.carryForward +
                          leaveSummary.types.ML.carryForward
                        ).toFixed(1)}
                      </td>
                      <td className="px-4 py-3 text-center text-gray-400 font-bold text-lg">
                        +
                      </td>
                      <td className="px-4 py-3 text-center text-sm text-blue-700">
                        {(
                          leaveSummary.types.CL.monthlyRemaining +
                          leaveSummary.types.EL.monthlyRemaining +
                          leaveSummary.types.ML.monthlyRemaining
                        ).toFixed(1)}
                      </td>
                      <td className="px-4 py-3 text-center text-gray-400 font-bold text-lg">
                        =
                      ...
                      <td className="px-4 py-3 text-center text-sm font-bold text-green-800">
                        {(
                          leaveSummary.types.CL.totalAvailable +
                          leaveSummary.types.EL.totalAvailable +
                          leaveSummary.types.ML.totalAvailable
                        ).toFixed(1)}
                      </td>
                      <td className="px-4 py-3 text-center text-sm text-red-600">
                        {(
                          leaveSummary.types.CL.yearlyUsed +
                          leaveSummary.types.EL.yearlyUsed +
                          leaveSummary.types.ML.yearlyUsed
                        ).toFixed(1)}
                      </td>
                      <td className="px-4 py-3 text-center text-sm text-gray-800">
                        {(
                          leaveSummary.types.CL.yearlyRemaining +
                          leaveSummary.types.EL.yearlyRemaining +
                          leaveSummary.types.ML.yearlyRemaining
                        ).toFixed(1)}
                      </td>
                    </tr>
                  </tbody>
        ...
        {showAddForm && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            {leaveSummary.remainingLeaves > 0 &&
              leaveSummary.remainingLeaves < 5 && (
            ...
            {leaveSummary.remainingLeaves <= 0 && (
              <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                <p className="text-sm text-orange-800 flex items-center">
                  ...
                  You have no leave balance remaining, but you can still submit
                  a request. It will be reviewed by your admin.
                </p>
              </div>
            )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className={`w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-200 ${
                        errors.leaveType
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className={`w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none transition-colors duration-200 ${
                        errors.reason
                          ? "border-red-500 bg-red-50"
                          : "border-gray-300"
                      }`}
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <span
                        className={`text-sm font-bold ${leaveSummary.remainingLeaves < 5 ? "text-red-600" : "text-green-600"}`}
                      >
                        {Number(leaveSummary.remainingLeaves).toFixed(1)} days
                      </span>
                    </div>
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <span
                        className={`text-sm font-bold ${leaveSummary.remainingLeaves - calculateDays(formData.fromDate, formData.toDate) < 0 ? "text-red-600" : "text-green-600"}`}
                      >
                        {(
                          Number(leaveSummary.remainingLeaves) -
                          ...
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <div className="text-gray-400 text-sm mt-1">
                              {searchTerm
                                ...
                                : "Get started by adding a new leave request"}
                            </div>
                          </div>
                        </td>
                      </tr>
              ...
              <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div className="text-sm text-gray-700">
                    Showing {paginatedLeaves.length > 0 ? startIndex + 1 : 0} to{" "}
                    {startIndex + paginatedLeaves.length} of{" "}
