...
const MonthlyWorkReport = () => {
    ...
    const fetchProjects = async () => {
      ...
      try {
              ...
              const assignments = allAssignments.filter(
                (a) =>
                  ...
                  String(a.projectbranch || "")
                    .trim()
                    .toLowerCase() ===
                    String(project.branch).trim().toLowerCase(),
              );
        ...
        setSelectedProjectKey((currentKey) => {
          if (
            currentKey &&
            transformedProjects.some((project) => project.key === currentKey)
          ) {
            return currentKey;
          }
          ...
          const activeMatch = transformedProjects.find(
        ...
        missingNameIds.map(async (id) => {
          try {
            const res = await axios.get(
              `${process.env.VITE_HRMS_BASE_URL}/api/tenant/hrms/get-employee/${id}`,
              {
                headers: {
            ...
            console.error(
              `Error fetching employee ${id} from Cloudsat API:`,
  ...
  const isReportInManagerScope = (report) => {
    ...
    if (reportProjectId && reportProjectBranch) {
      return managedProjectKeys.includes(
        getProjectKey(reportProjectId, reportProjectBranch),
      );
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !editingReport &&
      employeeProjectOptions.length > 0 &&
      !selectedReportProject
    ) {
      Swal.fire(
        "Error",
        "Please select the project for this monthly report",
        "error",
      );
      return;
    }
    ...
    try {
      if (editingReport) {
        ...
        await axios.post(`${API_BASE}/api/monthly-reports/${slug}/create`, {
          employee_id: employeeId,
          ...formData,
          ...projectPayload,
        });
  ...
  const handleReview = async (reportId, status) => {
    ...
    const reportProjectId = report.project_id || report.projectid;
    const reportProjectBranch = report.project_branch || report.projectbranch;
    const reportProjectKey = getProjectKey(
      reportProjectId,
      reportProjectBranch,
    );

    const isDirectProjectManager =
      managedProjectKeys.includes(reportProjectKey);
    ...
    if (
      ...
      user?.name !== report.projectManager
    ) {
      Swal.fire({
        icon: "error",
        title: "Permission Denied",
    ...
    if (!report || !canReviewReport(report)) {
      Swal.fire(
        "Error",
        "You can only review pending monthly reports from projects where you are assigned as Manager.",
        "error",
      );
      return;
    }
    ...
    if (comment !== undefined) {
      try {
        await axios.put(
          `${API_BASE}/api/monthly-reports/${slug}/admin/review/${reportId}`,
          {
            status,
        ...
        Swal.fire(
          "Error",
          error.response?.data?.error || "Review failed",
          "error",
        );
      }
    }
  ...
  const displayedReports = scopedReports.filter((r) => {
    ...
    const name =
      ...
      r.employee_name ||
      r.emp_name ||
      (id ? employeeMap[id]?.name : "") ||
      "";
    const department =
      r.department || r.dept || (id ? employeeMap[id]?.department : "") || "";
    ...
    const matchesYear = filterYear
      ? String(r.year) === String(filterYear)
      : true;
                    ...
                    return (
                      <React.Fragment key={report.id}>
                        <tr className="hover:bg-(--bg-app)/40 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-bold text-(--text-strong)">
                                {report.name ||
                                  ...
                                  report.emp_name ||
                                  employeeMap[
                                    report.employee_id || report.employeeId
                                  ]?.name ||
                                  "Loading..."}
                              </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2.5 py-0.5 text-[11px] font-semibold rounded-full flex items-center gap-1 w-fit ${getStatusBadge(report.status)}`}
                            >
                        ...
                        {expandedRows[report.id] && (
                          <tr className="bg-(--bg-app)/60">
                            <td
                              colSpan={canUseAdminReportView ? 6 : 5}
                              className="px-6 py-4"
                            >
                              <div className="bg-white border border-(--border-soft) p-4 rounded-xl space-y-4 shadow-sm">
                                <div>
                                  <h4 className="text-xs font-bold uppercase tracking-wider text-(--text-soft) mb-1">
