...
const MyRequest = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_CSAAP_URL}/api/tenant/hrms/all-employees`, {
  ...
  const handleAddRequest = async () => {
    ...
    if (!formData.request_to || !formData.request_details || !formData.request_type) {
          ...
          <div className="mb-8">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              <div>
                ...
                <p className="text-gray-600 text-lg">Manage and track all your service requests</p>
              </div>
              {canCreate && (
                <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-800 mt-1">{requests.length}</p>
                </div>
                <div className={`p-2 rounded-xl transition-colors ${statusFilter === 'All' ? 'bg-emerald-600 text-white' : 'bg-emerald-100 text-emerald-600'}`}>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-xl transition-colors ${statusFilter === 'Pending' ? 'bg-yellow-600 text-white' : 'bg-yellow-100 text-yellow-600'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-xl transition-colors ${statusFilter === 'In Progress' ? 'bg-emerald-600 text-white' : 'bg-emerald-100 text-emerald-600'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-xl transition-colors ${statusFilter === 'Completed' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-600'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <tr>
                        <td colSpan="9" className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center justify-center text-gray-400">
                            ...
                            <div className="text-lg font-medium text-gray-500 mb-2">No requests found</div>
                            <div className="text-sm text-gray-400 mb-4">Get started by creating your first request</div>
                            {canCreate && (
                              <button
                                onClick={() => setShowAddRequest(true)}
                      ...
                      paginatedRequests.map((request) => (
                        <tr key={request.id} className="hover:bg-gray-50/80 transition-colors group">
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${request.employee_id === employeeId
                                  ? 'bg-emerald-100 text-emerald-600 border border-emerald-200'
                                  : request.request_to_employee_id === employeeId
                                    ? 'bg-emerald-100 text-emerald-600 border border-emerald-200'
                          ...
                          <td className="px-6 py-4">
                            ...
                            <div className="text-[11px] text-gray-500">ID: {request.employee_id}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900">{request.request_to}</div>
                            {request.assigned_employee_name && (
                              <div className="text-[11px] text-gray-500 italic">({request.assigned_employee_name})</div>
                            )}
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900 max-w-xs line-clamp-2">{request.request_details}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1.5">
                              <div className={`w-2 h-2 rounded-full ${getPriorityColor(request.priority)}`} />
                              <span className="text-sm text-gray-600">{request.priority}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-600 max-w-xs">{request.reply_details}</div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              {request.employee_id === employeeId && canDelete && (
                                <button
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-gray-50/50">
                  <div className="flex flex-col lg:flex-row justify-between items-center gap-4 text-sm text-gray-600">
                    <div className="font-medium">
                      Showing {startIndex + 1} to {Math.min(startIndex + entriesPerPage, filteredRequests.length)} of {filteredRequests.length} requests
                    </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            <div className="bg-linear-to-r from-emerald-600 to-emerald-700 px-8 py-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Create New Request</h2>
                  <p className="text-emerald-100 mt-1">Fill in the details below to submit your service request</p>
                </div>
              </div>
            </div>
    ...
    <span>{formData.request_type.length}/50</span>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange('request_to', e.target.value)}
              ...
              {formData.request_to && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleInputChange('request_to_employee_id', e.target.value)}
                    className="w-full px-4 py-3.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all bg-white"
                  >
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-xl space-y-2 border border-gray-100">
                <p className="text-sm text-gray-600">
                  <span className="font-semibold text-gray-700">Type:</span> {selectedRequest.request_type}
                </p>
                <p className="text-sm text-gray-600">
                  <span className="font-semibold text-gray-700">Details:</span> {selectedRequest.request_details}
                </p>
              </div>
              <div>
      ...
      {showViewModal && selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 relative animate-fade-in">
            ...
            <h2 className="text-2xl font-bold mb-6 text-blue-700 border-b pb-4">Service Request Details</h2>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Type</p>
                  <p className="text-gray-900 font-medium">{selectedRequest.request_type}</p>
                </div>
                <div>
              ...
              <div>
                ...
                <p className="text-gray-900 font-medium">{selectedRequest.requester_name || 'Unknown'} (ID: {selectedRequest.employee_id})</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Request To</p>
                <p className="text-gray-900 font-medium">
                  {selectedRequest.request_to}
                  {selectedRequest.assigned_employee_name && (
                    <span className="text-gray-500 ml-1">({selectedRequest.assigned_employee_name})</span>
                  )}
                </p>
              </div>
              <div>
              ...
              <div>
                <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Details</p>
                <p className="text-gray-700 bg-gray-50 p-3 rounded-lg border border-gray-100 mt-1">{selectedRequest.request_details}</p>
              </div>
              {selectedRequest.reply_details && (
                <div>
                  <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Reply from HR/Admin</p>
                  <p className="text-purple-700 bg-purple-50 p-3 rounded-lg border border-purple-100 mt-1">{selectedRequest.reply_details}</p>
                </div>
              )}
