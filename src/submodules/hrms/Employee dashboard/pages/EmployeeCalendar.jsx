...
const EmployeeCalendar = () => {
  ...
  const fetchHolidays = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_HRMS_BASE_URL}/api/holiday`, {
  ...
  const fetchMeetings = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_HRMS_BASE_URL}/api/meetings`, {
        params: { company_id: user.company_id, slug: user.slug }
      });
  ...
  const sidebarMeetings = meetings.map((m, index) => {
    const colors = [
      ...
      { color: "from-purple-500 to-purple-600", lightColor: "bg-purple-50 text-purple-600" },
  ...
  const meetingsToday = meetings.filter(m => new Date(m.date).toLocaleDateString() === stringToday);

  const stats = [
    { label: "Team Members", value: "14", icon: Users, color: "from-blue-500 to-blue-600", lightColor: "bg-blue-50 text-blue-600", change: "+2", subtext: "Active members" },
    { label: "Next Holiday", value: nextHoliday ? new Date(nextHoliday.date).toLocaleDateString('en-US', {month: 'short', day: 'numeric'}) : "None", icon: Gift, color: "from-emerald-500 to-emerald-600", lightColor: "bg-emerald-50 text-emerald-600", change: "", subtext: nextHoliday ? nextHoliday.name : "No upcoming holidays" },
    { label: "Scheduled Tasks", value: "08", icon: FileText, color: "from-indigo-500 to-indigo-600", lightColor: "bg-indigo-50 text-indigo-600", change: "+3", subtext: "This week" },
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className={`p-1.5 sm:p-2 rounded-lg ${stat.lightColor} group-hover:scale-105 transition-transform`}>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm p-3 sm:p-4">
            <BigCalendar
              ...
              eventPropGetter={(event) => {
                let backgroundColor = event.type === 'holiday' ? '#10B981' : '#3B82F6';
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-linear-to-r from-slate-50 to-white">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-800">Schedule Meetings</h3>
                  <p className="text-[10px] text-slate-400 mt-0.5">Today & Upcoming</p>
                </div>
            ...
            <div className="divide-y divide-slate-100 max-h-87.5 overflow-y-auto">
              {sidebarMeetings.map((meeting) => (
                <div key={meeting.id} className="p-4 hover:bg-slate-50 transition-colors group">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-lg ${meeting.lightColor} group-hover:scale-105 transition-transform`}>
                      <Video size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[10px] text-slate-500">{meeting.attendees} attendees</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        <MapPin size={10} className="text-slate-400" />
                        <span className="text-[9px] text-slate-400">{meeting.location}</span>
                      </div>
                    </div>
      ...
      >
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">Holiday Calendar {selectedYear}</h2>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Annual Observances</p>
            </div>
          </div>
        ...
        <div className="px-4 py-4 bg-white border-t border-slate-100 flex items-center justify-between">
          <div className="flex gap-4">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Public Holiday</span>
             </div>
             <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Observance</span>
             </div>
          </div>
