...
import { 
  Key, Shield, Plus, Edit2, Trash2, 
  Loader2, Save, X, CheckSquare, Square, Layers, Info, Search
} from 'lucide-react';
...
const PermissionManager = () => {
  ...
  const filteredMasterPermissions = useMemo(() => {
    ...
    return Object.keys(groupedPermissions).reduce((acc, module) => {
      ...
      const filteredPerms = groupedPermissions[module].filter(perm => 
  ...
  const handlePermSubmit = async (e) => {
    ...
    try {
      if (editingPermId) {
        await axios.put(`${permissionsApiUrl}/${editingPermId}`, formData, getAuthHeaders());
  ...
  const handleSelectAllInModule = (moduleName, customCodes = null) => {
    const moduleCodes = customCodes || groupedPermissions[moduleName].map(p => p.code);
                      ...
                      {filteredMasterPermissions[module].map(perm => (
                        <div key={perm.id} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg group transition-colors">
                          <div>
                            ...
                            <p className="text-[9px] font-mono text-indigo-400 mt-0.5">{perm.code}</p>
                          </div>
                  ...
                  >
                    ...
                    {roles.map(role => (
                      <option key={role.id} value={role.id}>
                        {role.role_name} {role.department_name ? `(${role.department_name})` : '(Global)'}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            {!selectedRoleId ? (
              ...
              <div className="space-y-8">
                {Object.keys(filteredMatrixPermissions).map(module => {
                  const moduleCodes = filteredMatrixPermissions[module].map(p => p.code);
                  const allAssigned = moduleCodes.every(code => assignedCodes.includes(code));
                  const someAssigned = moduleCodes.some(code => assignedCodes.includes(code));
                  ...
                  return (
                    <div key={module} className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                      ...
                      >
                        <h3 className={`text-sm font-black uppercase tracking-widest ${someAssigned ? 'text-indigo-900' : 'text-slate-600'}`}>
                          Module: {module}
                        </h3>
                        <button type="button" className={`flex items-center gap-2 text-xs font-bold transition-colors ${allAssigned ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                          {allAssigned ? <CheckSquare size={16} /> : <Square size={16} />}
                          Select All
                        </button>
                      </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex flex-col">
                                <span className={`text-sm font-bold ${isChecked ? 'text-indigo-900' : 'text-slate-700'}`}>{perm.action}</span>
                                <span className="text-[9px] font-mono text-slate-400 mt-0.5">{perm.code}</span>
                                {perm.description && (
                                  <p className="text-xs font-medium text-slate-500 mt-1.5 flex items-start gap-1">
                                    <Info size={12} className="shrink-0 mt-0.5" /> {perm.description}
                                  </p>
                                )}
                              </div>
                            </label>
