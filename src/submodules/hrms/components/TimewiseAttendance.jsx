...
export default function TimewiseAttendance() {
  ...
  const summary = {
    ...
    totalPresent: attendanceData.reduce(
      (total, emp) =>
        total + emp.days.filter((day) => day.status === "Present").length,
      0,
    ),
    totalAbsent: attendanceData.reduce(
      (total, emp) =>
        total + emp.days.filter((day) => day.status === "Absent").length,
      0,
    ),
                ...
                <tbody>
                  {filteredEmployees.map((employee) => (
                    <React.Fragment key={employee.empCode}>
                      ...
                      {["In Time", "Out Time", "Late By", "Early By"].map(
                        (detailType) => (
