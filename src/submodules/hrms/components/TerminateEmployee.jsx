...
import { Trash2, Search, ChevronLeft, ChevronRight, UserCheck, X } from "lucide-react";
...
const TerminationFormModal = ({ employee, onClose, onTerminate }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (window.confirm(`Are you sure you want to terminate ${employee.name}? This action cannot be undone.`)) {
      onTerminate({
        ...
        employee: {
          ...
          designation: employee.designation || employee.postApplied || employee.position || "N/A",
        },
      });
      onClose();
    }
  ...
  return (
    <div className="app-modal-backdrop fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="app-modal max-w-md w-full p-6 border border-(--border-strong) bg-white shadow-2xl rounded-[22px]" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center border-b border-(--border-soft) pb-4 mb-4">
          <h2 className="text-base font-extrabold text-(--text-strong) tracking-tight">Terminate Employee</h2>
          <button onClick={onClose} className="p-1.5 text-(--text-soft) hover:text-(--text-strong) rounded-lg hover:bg-(--bg-subtle) transition-colors cursor-pointer">
            <X size={18} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-(--text-strong) mb-1">Termination Type</label>
            <select
          ...
          <div>
            <label className="block text-xs font-bold text-(--text-strong) mb-1">Date</label>
            <input
              type="date"
              value={date}
          ...
          <div>
            <label className="block text-xs font-bold text-(--text-strong) mb-1">Remark</label>
            <textarea
              value={remark}
          ...
          <div>
            <label className="block text-xs font-bold text-(--text-strong) mb-1">Notice Period (Days)</label>
            <input
              type="number"
              min="0"
              value={noticePeriod}
            ...
            <p className="text-[10px] font-medium text-(--text-soft) mt-1 flex items-center gap-1">
              ...
              <span>Enter 0 for immediate termination (marks as Ex-Employee).</span>
            </p>
          </div>
...
export default function TerminateEmployeePage() {
  ...
  async function fetchEmployees() {
    try {
      const res = await axios.get(`${import.meta.env.VITE_HRMS_BASE_URL}/api/terminate`);
      ...
      <div className="mx-auto max-w-7xl px-3 py-4 lg:px-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-2">
          <div>
            ...
            <p className="app-subtitle mt-1">Manage and track company separations and terminations</p>
          </div>
        ...
        <div className="app-panel p-6 border border-(--border-soft) bg-white">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <div className="flex items-center gap-2">
              <label htmlFor="entries" className="text-xs font-bold text-(--text-soft)">
                Show
              </label>
              <select
                id="entries"
              ...
              <span className="text-xs font-bold text-(--text-soft)">entries</span>
            </div>

            <div className="relative w-full md:w-64">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-(--text-faint)" size={16} />
              <input
                type="text"
                value={search}
                onChange={(e) => {
              ...
              <tbody className="divide-y divide-(--border-soft)">
                {currentEmployees.length > 0 ? (
                  currentEmployees.map((emp) => (
                    <tr key={emp.id} className="hover:bg-(--bg-subtle)/40 transition-all duration-200">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            <div className="font-extrabold text-[13px] text-(--text-strong)">{emp.name}</div>
                            <div className="text-[10px] font-bold text-(--text-soft)">ID: {emp.id}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {emp.status === "Active" ? (
                          ...
                          <span className="text-red-600 font-extrabold text-xs">Terminated</span>
                        )}
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan="4" className="px-6 py-12 text-center bg-white">
                      <div className="text-(--text-soft) flex flex-col items-center justify-center">
                        ...
                        <p className="font-bold text-sm text-(--text-strong)">No employees found</p>
                        <p className="text-xs text-(--text-soft) mt-1">Try adjusting your search or filter</p>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mt-6 pt-6 border-t border-(--border-soft)">
            <div className="text-xs font-bold text-(--text-soft) mb-4 md:mb-0">
              ...
              <span className="text-(--text-strong)">{Math.min(endIndex, filtered.length)}</span> of{" "}
              <span className="text-(--text-strong)">{filtered.length}</span> results
            </div>
