...
const DocumentCard = ({
  ...
  return (
    <div className="app-panel border border-(--border-soft) rounded-xl p-4 hover:shadow-md transition-all duration-200">
      ...
      <div
        className="text-xs text-(--text-soft) mb-3 truncate"
        title={fileName}
      >
        📄 {fileName}
      </div>
...
const ViewJobApplication = ({ employeeId, employeeData, onClose }) => {
  ...
  useEffect(() => {
    ...
    setError(
      employeeData || employeeId ? null : "Employee data could not be found.",
    );
  ...
  const getFileIcon = (fileUrl) => {
    ...
    switch (type) {
      ...
      default:
        return <FileText size={16} className="text-(--text-soft)" />;
    }
  };
      ...
      case "pdf":
        return (
          <div className="w-full h-96">
            <iframe
              src={previewUrl}
              className="w-full h-full border border-(--border-soft) rounded-xl"
              title="PDF Preview"
            />
          </div>
        );
        ...
        return (
          <div className="flex flex-col items-center justify-center h-64 bg-(--bg-subtle) rounded-xl">
            ...
            <button
              ...
              className="app-btn-primary mt-4 cursor-pointer"
            >
              Download File
            </button>
          </div>
  ...
  const Info = ({ label, value }) => (
    <div className="app-panel border border-(--border-soft) rounded-xl p-4 hover:shadow-sm transition-all duration-200">
      <label className="block text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft) mb-1">
        {label}
      </label>
  ...
  const Th = ({ children }) => (
    <th className="px-5 py-3 text-left text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft)">
      {children}
    </th>
  );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center app-modal-backdrop">
        <div className="app-modal p-8 max-w-md w-full mx-4 text-center">
          ...
          <button
            onClick={onClose}
            className="app-btn-primary px-6 cursor-pointer"
          >
            Go Back
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-50 overflow-auto p-4 md:p-6 app-modal-backdrop">
      <div className="app-modal max-w-6xl w-full mx-auto my-10 p-0! overflow-hidden flex flex-col">
                ...
                <table className="min-w-full divide-y divide-(--border-soft)">
                  ...
                  <tbody className="divide-y divide-(--border-soft)">
                    {parseJSON(employee.education).map((edu, i) => (
                      <tr
                        key={i}
                        className="hover:bg-(--bg-subtle)/70 transition-all duration-200"
                      >
                        <Td>{edu.course}</Td>
                        <Td>{edu.board}</Td>
              ...
              {!hasDocuments ? (
                ...
                <div className="space-y-8">
                  {documentCategories.map((cat, i) => {
                    const docs = cat.documents.filter((d) =>
                      d.isMultiple ? d.files.length > 0 : d.file,
                    );
                    if (docs.length === 0) return null;
                    return (
                      <div
                        key={i}
                        className="app-panel border border-(--border-soft) rounded-xl overflow-hidden"
                      >
              ...
              {monthlyReports.length === 0 && !loadingReports ? (
                <div className="text-center py-16 bg-(--bg-subtle) rounded-xl border border-dashed border-(--border-soft)">
                  ...
                  <p className="text-sm text-(--text-soft)/70 mt-1">
                    No submissions for this employee
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {monthlyReports.map((report, i) => (
                    <div
                      key={i}
                      className="group app-panel border border-(--border-soft) rounded-xl hover:shadow-lg hover:border-(--brand)/30 transition-all duration-200 overflow-hidden"
                    >
                      <div className="relative">
                        <div className="absolute top-0 right-0 m-3">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 shadow-sm capitalize ${
                        ...
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg, hsl(var(--bg-subtle)), hsl(var(--brand) / 0.08))",
                          }}
                        >
        ...
        <div className="border-t border-(--border-soft) p-6 bg-(--bg-subtle) flex justify-end">
          <button
            onClick={onClose}
            className="app-btn-secondary px-6 cursor-pointer"
          >
            Close
          </button>
        </div>
      ...
      {selectedDocument && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 app-modal-backdrop">
          <div
            className="app-modal max-w-4xl w-full p-0! overflow-hidden"
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-(--border-soft)">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => handleDownload(previewUrl)}
                  className="app-btn-primary p-2! cursor-pointer"
                >
                  <Download size={20} />
                </button>
                <button
      ...
      {viewText && (
        <div className="fixed inset-0 z-70 flex items-center justify-center p-4 app-modal-backdrop">
          <div
            className="app-modal max-w-2xl w-full p-0! overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b border-(--border-soft)">
