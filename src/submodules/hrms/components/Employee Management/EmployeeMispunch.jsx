...
// Locked to the established design language: Rings over shadows, heavy rounding.
const panelClass =
  "bg-white rounded-3xl shadow-sm ring-1 ring-slate-200 overflow-hidden transition-all hover:shadow-md";

const controlClass =
  "w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-700 font-medium placeholder-slate-400";
...
// Pastel backgrounds + matching text + matching soft border
...
const EmployeeMispunch = () => {
  ...
  const fetchMispunchData = useCallback(async () => {
    try {
      setLoading(true);
      ...
      console.error("Mispunch fetch error:", error);
      setRecords([]);
    } finally {
      setLoading(false);
    }
  ...
  const bulkUpdateMispunchStatus = async (status) => {
    ...
    const confirmed = window.confirm(
      `Do you want to ${actionLabel} ${selectedCount} selected mispunch record(s)?`,
    );
    if (!confirmed) return;
    ...
    try {
      ...
      console.error(`Failed to bulk ${actionLabel} mispunches`, error);
      alert(
        `Some mispunch records could not be ${actionLabel}d. Please try again.`,
      );
    } finally {
  ...
  const updateMispunchStatus = async (record, status, event) => {
    ...
    const actionLabel = status === "Approved" ? "approve" : "reject";
    const confirmed = window.confirm(
      `Do you want to ${actionLabel} the mispunch for ${record.employeeName} on ${record.date}?`,
    );
    if (!confirmed) return;
    ...
    try {
      ...
      console.error(`Failed to ${actionLabel} mispunch`, error);
      alert(`Unable to ${actionLabel} this mispunch right now.`);
    } finally {
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ...filteredRecords.map((item) => [
        ...
        item.employeeId,
        item.employeeName,
        formatTimeOnly(item.mispunchTime),
        formatTimeOnly(item.leaveTime),
        item.totalHoursLabel,
    ...
    link.href = url;
    link.download = `employee-mispunch-${monthQuery}.csv`;
    link.click();
  ...
  const renderFlagBadge = (label, active) => {
        ...
        <div className="space-y-4 text-center">
          ...
          <div>
            <p className="text-lg font-bold text-slate-900">
              Loading mispunch records
            </p>
            <p className="text-sm text-slate-500 font-medium">
              Syncing punch data for {monthNames[selectedMonth]} {selectedYear}
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-indigo-100 bg-indigo-50/50 px-5 py-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-indigo-900">
                  mispunch{selectedCount !== 1 ? "es" : ""} selected
                </span>
              </div>
            ...
            {filteredRecords.length === 0 ? (
              <div className="flex h-full flex-col items-center justify-center text-center py-12">
                ...
                <p className="text-slate-500 text-sm max-w-sm font-medium">
                  No attendance entries matched the current filters for this
                  month.
                </p>
              </div>
            ) : (
                          ...
                          {group.records.map((record) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3 min-w-45">
                                <div className="flex items-center gap-1 text-xs font-medium text-slate-600">
                                  ...
                                  <span className="font-mono">
                                    {formatTimeOnly(record.mispunchTime)}
                                  </span>
                                </div>
                                <div className="flex items-center gap-1 text-xs font-medium text-slate-600">
                              ...
                              <div className="flex flex-wrap items-center gap-2 shrink-0">
                                ...
                                {renderFlagBadge("Late", record.isLate)}
                                {renderFlagBadge("Half Day", record.isHalfDay)}
                                {renderFlagBadge(
                                  "Early Leave",
                                  record.isEarlyLeave,
                                )}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3">
                    ...
                    <div className="px-4 py-3 border-b border-slate-100 sm:border-r">
                      ...
                      <p className="text-sm font-bold text-slate-900 font-mono">
                        {formatTimeOnly(selectedRecord.mispunchTime)}
                      </p>
                    </div>
                    {/* Leave Time */}
                    <div className="px-4 py-3 border-b border-slate-100 sm:border-r">
                    ...
                    {/* Duration */}
                    <div className="px-4 py-3 border-b border-slate-100 col-span-2 sm:col-span-1">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">
                        Duration
                      </p>
                      <p className="text-sm font-bold text-slate-900">
...
export default EmployeeMispunch;
