...
const NoticeFormModal = ({ employee, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_HRMS_BASE_URL}/api/noticeletters/add`,
        {
...
export default function NoticeLetterPage() {
  ...
  function handleSendNotice(id) {
    if (
      window.confirm("Are you sure you want to send a notice to this employee?")
    ) {
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Manage and send notices to employees
            </p>
          </div>
