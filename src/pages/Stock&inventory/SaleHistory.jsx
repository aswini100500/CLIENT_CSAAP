...
const SaleHistory = () => {
  ...
  const handleDeleteSale = async (saleId) => {
    if (!window.confirm('Are you sure you want to delete this sale? The stock will be restored.')) {
      return;
    }
            ...
            <div className="flex-1 flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-700">
                  Showing <span className="font-medium">{filteredData.length}</span> entries for {month} {year}
                </p>
              </div>
              <div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedSale.invoice_no}</p>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Gross Price</label>
                    <p className="mt-1 text-sm text-gray-900">{formatCurrency(parseFloat(selectedSale.gross_price))}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Net Price</label>
                    <p className="mt-1 text-sm font-semibold text-gray-900">{formatCurrency(parseFloat(selectedSale.net_price))}</p>
                  </div>
                </div>
                ...
                {editMode ? (
                  <>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditData({...editData, to_account: e.target.value})}
                        className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      />
                    </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Paid Amount</label>
                        <p className="mt-1 text-sm text-gray-900">{formatCurrency(parseFloat(selectedSale.paid_amount))}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">To Account</label>
                        <p className="mt-1 text-sm text-gray-900">{selectedSale.to_account || 'Not specified'}</p>
                      </div>
                    </div>
