...
const PurchaseHistory = () => {
  ...
  const filteredPurchases = purchases.filter((purchase) => {
    ...
    return (
      (purchase.bill_no &&
        purchase.bill_no.toLowerCase().includes(searchLower)) ||
      (purchase.supplier_name &&
        purchase.supplier_name.toLowerCase().includes(searchLower)) ||
      (purchase.store_name &&
        purchase.store_name.toLowerCase().includes(searchLower)) ||
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute(
      "download",
      `purchase_history_${month || "all"}_${year || "all"}.csv`,
    );
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-800">
                  {formatCurrency(
                    purchases.reduce(
                      (sum, purchase) =>
                        sum + parseFloat(purchase.net_price || 0),
                      0,
                    ),
                  )}
                </div>
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-800">
                  {formatCurrency(
                    purchases.reduce(
                      (sum, purchase) =>
                        sum + parseFloat(purchase.paid_amount || 0),
                      0,
                    ),
                  )}
                </div>
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-800">
                  {formatCurrency(
                    purchases.reduce(
                      (sum, purchase) =>
                        sum + parseFloat(purchase.pending_amount || 0),
                      0,
                    ),
                  )}
                </div>
            ...
            {currentPurchases.length > 0 ? (
              currentPurchases.map((purchase) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div
                      className={`text-sm font-medium ${parseFloat(purchase.pending_amount || 0) > 0 ? "text-red-600" : "text-green-600"}`}
                    >
      ...
      {currentPurchases.length > 0 && (
        <div className="flex items-center justify-between mt-4 px-6 py-3 bg-gray-50 border-t border-gray-200 rounded-b-lg">
          <div className="text-sm text-gray-700">
            ...
            of <span className="font-medium">{filteredPurchases.length}</span>{" "}
            entries
          </div>
