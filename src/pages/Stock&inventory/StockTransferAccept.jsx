...
const StockTransferAccept = () => {
  ...
  const filteredTransfers = allTransfers.filter(transfer => {
    ...
    const productName = products.find(product => product.id === transfer.product_id)?.name || '';
  ...
  const handleAcceptTransfer = async (transferId) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to accept transfer. Please try again.';
  ...
  const handleRejectTransfer = async (transferId) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to reject transfer. Please try again.';
      showNotification('error', 'Error', errorMessage);
    } finally {
      setIsProcessing(false);
    }
                  ...
                  <tr key={transfer.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        transfer.status === 'accepted' || transfer.status === 'Accepted'
                          ? 'bg-green-100 text-green-800'
                          : transfer.status === 'pending' || transfer.status === 'Pending'
                          ? 'bg-yellow-100 text-yellow-800'
                          : transfer.status === 'rejected' || transfer.status === 'Rejected'
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      selectedTransfer.status === 'accepted' || selectedTransfer.status === 'Accepted'
                        ? 'bg-green-100 text-green-800'
                        : selectedTransfer.status === 'pending' || selectedTransfer.status === 'Pending'
                        ? 'bg-yellow-100 text-yellow-800'
                        : selectedTransfer.status === 'rejected' || selectedTransfer.status === 'Rejected'
                        ? 'bg-red-100 text-red-800'
                        : 'bg-gray-100 text-gray-800'
                    }`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{getStoreName(selectedTransfer.from_store_id)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">To Store</p>
                    <p className="font-medium">{getStoreName(selectedTransfer.to_store_id)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{getProductName(selectedTransfer.product_id)}</p>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedTransfer.quantity} units</p>
                  </div>
                </div>
                ...
                {selectedTransfer.requested_by && (
                  <div>
                    ...
                    <p className="font-medium">{selectedTransfer.requested_by}</p>
                  </div>
                )}
                ...
                {selectedTransfer.completed_at && (
                  <div>
                    ...
                    <p className="font-medium">{formatDate(selectedTransfer.completed_at)}</p>
                  </div>
                )}
