...
const StockList = () => {
  ...
  const stockEntries = stockEntriesData?.success
    ? stockEntriesData.data || []
    : [];
  ...
  const handleDamageSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        error.message ||
        "Failed to report damage. Please try again.";
  ...
  const countLowStockItems = () => {
    return filteredData.filter((item) => parseFloat(item.quantity || 0) < 10)
      .length;
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-red-50 border border-red-200 rounded-md p-4">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Note: The API endpoint for stock entries might be different.
              Please check if the endpoint <code>/api/tenant/stock/entry</code>{" "}
              is correct.
            </p>
            <button
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-4">
          <p className="text-sm text-yellow-800">
            Debug Info: Loaded {stores.length} stores, {products.length}{" "}
            products, and {stockEntries.length} stock entries.
          </p>
        </div>
          ...
          {stockEntries.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm text-gray-600">
                Add stock entries from the "Stock Entry" tab to see them here.
              </p>
            </div>
          ) : filteredData.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <p className="mb-2">
                No matching stock entries found for "{searchTerm}".
              </p>
              <button
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentEntries.map((item, index) => (
                      <tr key={item.id || index} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          ₹{parseFloat(item.purchase_amount || 0).toFixed(2)}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              <h2 className="text-xl font-bold text-gray-800 mb-4">
                Barcode for {selectedItem.product_name}
              </h2>
              <div className="bg-white p-4 border border-gray-300 rounded-md mb-4 flex justify-center">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500">
                    Batch: {selectedItem.batch} | Rack: {selectedItem.rack} |
                    Stock: {selectedItem.quantity}
                  </p>
                  <p className="text-xs text-gray-500">
                    MRP: ₹{parseFloat(selectedItem.mrp || 0).toFixed(2)} | Sale:
                    ₹{parseFloat(selectedItem.sale_price || 0).toFixed(2)}
                  </p>
                </div>
              </div>
      ...
      {showDamageModal && selectedItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              <h2 className="text-xl font-bold text-gray-800 mb-4">
                Damage Entry for {selectedItem.product_name}
              </h2>
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Current stock: {selectedItem.quantity} {selectedItem.units}
                </p>
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  ...
                  disabled={
                    damageQuantity <= 0 ||
                    damageQuantity > (selectedItem.quantity || 0) ||
                    !damageReason
                  }
