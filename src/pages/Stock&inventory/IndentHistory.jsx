...
const IndentHistory = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR("/api/tenant/indents/history", postFetcher, {
    revalidateOnFocus: false,
    shouldRetryOnError: false,
  });

  const indents = historyData?.success ? historyData.data?.indents || [] : [];
  const pagination = historyData?.success
  ...
  const currentIndents = filteredIndents.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  ...
  const updateIndentStatus = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `/api/tenant/indents/status/${selectedIndent.id}`,
        {
          status: status,
        },
      );
  ...
  const deleteIndent = async () => {
    ...
    try {
      ...
      const response = await api.delete(
        `/api/tenant/indents/${selectedIndent.id}`,
      );
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="text-sm font-medium text-gray-900">
                              {indent.supplier_name}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="text-sm text-gray-600">
                              {indent.supplier_contact || "N/A"}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="text-sm text-gray-600">
                              {indent.supplier_gst || "N/A"}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadgeColor(indent.status)}`}
                            >
                              {indent.status?.charAt(0).toUpperCase() +
                                indent.status?.slice(1) || "Draft"}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="text-sm text-gray-900">
                              {indent.total_items || 0}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                      ...
                      <tr>
                        <td colSpan="8" className="px-4 py-12 text-center">
                          <div className="flex flex-col items-center justify-center text-gray-500">
                            ...
                            <p className="text-sm">
                              No indents available or matching your search
                              criteria
                            </p>
                          </div>
                        </td>
