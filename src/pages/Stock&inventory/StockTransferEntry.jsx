...
const StockTransferEntry = () => {
  ...
  const unitsFromApi = master.units || [
    "Pieces",
    "Boxes",
    "Packets",
    "Units",
  ...
  const { data: stockEntriesData, isLoading: stockEntriesLoading } = useSWR(
    fromStore && product
      ? `/api/tenant/stock/entry?store_id=${fromStore}&product_id=${product}`
      : null,
    fetcher,
    {
      revalidateOnFocus: false,
  ...
  const getAvailableQuantity = () => {
    ...
    return stockEntries.reduce(
      (total, entry) => total + parseFloat(entry.quantity || 0),
      0,
    );
  };
  ...
  const handleTransfer = async () => {
    if (!fromStore || !toStore || !product || !quantity) {
      showNotification(
        "error",
        "Validation Error",
        "Please fill all required fields: From Store, To Store, Product, and Quantity",
      );
      return;
    }
    ...
    if (!requestedBy) {
      showNotification(
        ...
        "Unable to identify user. Please ensure you are logged in.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await api.post(
        "/api/tenant/stock/transfer/request",
        transferData,
      );
      ...
      if (response.data.success) {
        ...
        throw new Error(
          response.data.message || "Failed to create transfer request",
        );
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
          ...
          <label className="block text-sm font-medium text-gray-700">
            ...
            {fromStore && product && (
              <span className="ml-2 text-xs text-blue-600">
                ...
                {products.find((p) => p.id == product)?.units || "units"}
              </span>
            )}
          </label>
          <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          ...
          {category && !fromStore && (
            <p className="text-xs text-gray-500 mt-1">
              Please select from store to check availability
            </p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          {product &&
            quantity &&
            parseFloat(quantity) > getAvailableQuantity() && (
      ...
      <div className="mb-6">
        ...
        />
        <p className="text-xs text-gray-500 mt-1">
          Optional: Add any special instructions or comments for this stock
          transfer.
        </p>
      </div>
      ...
      <div className="mb-6 p-3 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center text-sm text-gray-600">
          ...
          <span>
            Request will be submitted under your account:{" "}
            <span className="font-medium">
              {requestedBy || "Loading user..."}
            </span>
          </span>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {fromStore && (
              <div>
                ...
                <p className="font-medium">
                  {stores.find((s) => s.id == fromStore)?.name ||
                    "Not selected"}
                </p>
              </div>
            )}
            {toStore && (
              <div>
                ...
                <p className="font-medium">
                  {stores.find((s) => s.id == toStore)?.name || "Not selected"}
                </p>
              </div>
            )}
            {product && (
