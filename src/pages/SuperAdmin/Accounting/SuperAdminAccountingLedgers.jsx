...
const SuperAdminAccountingLedgers = ({ hideHeader }) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Unable to load employee-created ledgers.",
      );
      setData({ activity: [] });
    } finally {
      setLoading(false);
    ...
    if (searchTerm.trim()) {
      ...
      list = list.filter(
        (item) =>
          ...
          (item.creator_email &&
            item.creator_email.toLowerCase().includes(term)),
      );
    }
    return list;
  ...
  const stats = useMemo(() => {
    ...
    const activeCreators = new Set(
      ledgers
        .map((l) => l.created_by_employee_id || l.created_by_user_id)
        .filter(Boolean),
    );
  ...
  const handleView = async (item) => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.ACCOUNTING_API_URL}/api/v1/ledger/${item.company_id}/${item.record_id}`,
      );
  ...
  return (
    <div
      className={
        hideHeader
          ? "w-full space-y-4 p-2 bg-white relative"
          : "min-h-screen bg-slate-50 p-3 sm:p-5 relative"
      }
    >
      <div className={hideHeader ? "space-y-4" : "mx-auto max-w-7xl space-y-5"}>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {companyName
                    ? `Viewing ledgers for ${companyName}`
                    : "Review account ledgers created by employees across companies."}
                </p>
              </div>
        ...
        <div className="p-5 rounded-lg border border-slate-200/80 bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xs font-bold uppercase text-slate-500 tracking-wide">
              Filters
            </h3>
            {hideHeader && (
              <button
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <StatCard
            icon={BookOpen}
            label="Total Ledgers Created"
            value={stats.totalCount}
            tone="blue"
          />
          <StatCard
            icon={IndianRupee}
            label="Cumulative Balances"
            value={formatAmount(stats.totalBalances)}
            tone="green"
          />
          <StatCard
            icon={UserRound}
            label="Ledger Creators"
            ...
            <table className="w-full min-w-245 text-left text-sm font-normal">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  ledgers.map((item) => (
                    <tr
                      key={`${item.resource_key}-${item.record_id}`}
            ...
            <div className="p-6 max-h-[80vh] overflow-y-auto">
              <div className="mb-6 rounded-lg bg-green-50 p-4 border border-green-100">
                ...
                <p className="text-lg font-bold text-slate-900">
                  {selectedLedger.company_name ||
                    `ID: ${selectedLedger.company_id}`}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  <p className="font-medium text-slate-800">
                    {selectedLedger.creator_name ||
                      selectedLedger.creator_email ||
                      "System"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium text-slate-800">
                    {selectedLedger.created_by_employee_id || "--"}
                  </p>
                </div>
              </div>

              <div className="border-t border-slate-100 pt-6">
                <h4 className="text-sm font-bold uppercase text-slate-800 mb-4">
                  Balances
                </h4>
                {loadingDetails ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="rounded-lg border border-slate-200 p-4 bg-slate-50">
                      ...
                      <p className="text-xl font-extrabold text-slate-900">
                        {formatAmount(ledgerDetails.openingBalance)}
                      </p>
                    </div>
                    <div className="rounded-lg border border-green-200 p-4 bg-green-50">
                      ...
                      <p className="text-xl font-extrabold text-green-900">
                        {formatAmount(ledgerDetails.closingBalance)}
                      </p>
                    </div>
                    <div className="rounded-lg border border-slate-200 p-4 bg-slate-50">
                      <p className="text-xs font-bold uppercase text-slate-500 mb-2">
                      ...
                      <p className="font-bold text-slate-700">
                        {formatAmount(ledgerDetails.debit)}
                      </p>
                    </div>
                    <div className="rounded-lg border border-slate-200 p-4 bg-slate-50">
                      <p className="text-xs font-bold uppercase text-slate-500 mb-2">
                      ...
                      <p className="font-bold text-slate-700">
                        {formatAmount(ledgerDetails.credit)}
                      </p>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="border-t border-slate-100 pt-6 mt-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-slate-800">
                          {ledgerDetails.registrationType || "--"}
                        </p>
                      </div>
                    </div>
                  </div>
