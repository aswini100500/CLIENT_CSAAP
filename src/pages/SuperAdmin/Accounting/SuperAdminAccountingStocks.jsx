...
const SuperAdminAccountingStocks = ({ hideHeader }) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Unable to load employee-created stock items.",
      );
      setData({ activity: [] });
    } finally {
      setLoading(false);
  ...
  const stats = useMemo(() => {
    ...
    const activeCreators = new Set(
      stocks
        .map((s) => s.created_by_employee_id || s.created_by_user_id)
        .filter(Boolean),
    );
    ...
    >
      <div className={hideHeader ? "space-y-4" : "mx-auto max-w-7xl space-y-5"}>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {companyName
                    ? `Viewing stock items for ${companyName}`
                    : "Review stock items and inventory records created by employees."}
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <StatCard
            icon={Boxes}
            label="Cumulative Opening Balance Value"
            value={stats.totalOpeningBalance.toLocaleString("en-IN")}
            tone="green"
          />
          <StatCard
            icon={UserRound}
            label="Stock Creators"
