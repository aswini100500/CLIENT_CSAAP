...
const PlansPage = () => {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-slate-800">Plans & Subscription</h1>
      ...
      {loading && (
        <div className="flex justify-center items-center py-12">
          <div className="text-slate-600">Loading plans and company information...</div>
        </div>
      )}
        ...
        <>
          ...
          <div className="mb-6 bg-white p-6 rounded-lg shadow-md border border-slate-200">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-sm font-medium text-slate-600">Number of Users</p>
                <p className="text-2xl font-bold text-blue-600">
                  {company.number_of_users}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium text-slate-600">Number of Projects</p>
                <p className="text-2xl font-bold text-green-600">
                  {company.number_of_projects}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium text-slate-600">Plan Duration (days)</p>
          ...
          {matchedPlan && (
            <div className="mb-6 bg-linear-to-r from-green-50 to-emerald-50 p-6 rounded-lg shadow-md border-2 border-green-500">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <p className="text-sm font-medium text-slate-600 mb-2">Plan Name</p>
                  <p className="text-lg font-bold text-slate-800">{matchedPlan.description}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-slate-600 mb-2">Price</p>
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold text-slate-800">
                    {matchedPlan.user} users / {matchedPlan.numberproject} projects / {matchedPlan.validity} days
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium text-slate-600 mb-2">Match Score</p>
                  <p className="text-lg font-semibold text-slate-800">
                    Users: {matchedPlan.user >= company.number_of_users ? '✓' : '✗'} |
                    Projects: {matchedPlan.numberproject >= company.number_of_projects ? '✓' : '✗'} |
                    Duration: {matchedPlan.validity >= company.plan_duration / 2 ? '✓' : '✗'}
                  </p>
                </div>
              </div>
            </div>
          ...
          {!matchedPlan && (
            <div className="mb-6 bg-yellow-50 p-4 rounded-lg border border-yellow-200">
              <p className="text-yellow-800 font-medium">
                ⚠ No perfect plan match found. Please review available plans below.
              </p>
            </div>
          )}
              ...
              {plans.map((plan) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-slate-800 mb-3">{plan.description}</h3>

                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-600">Projects:</span>
                        <span className="font-semibold text-slate-800">{plan.numberproject}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-600">Users:</span>
                        <span className="font-semibold text-slate-800">{plan.user}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-600">Validity:</span>
                        <span className="font-semibold text-slate-800">{plan.validity} days</span>
                      </div>
                    ...
                    <div className="space-y-1 text-xs mb-4">
                      {plan.user >= company.number_of_users && (
                        <div className="text-green-600 flex items-center">
                          <span className="mr-1">✓</span> Covers {company.number_of_users} users
                        </div>
                      )}
                      {plan.user < company.number_of_users && (
                      ...
                      {plan.numberproject >= company.number_of_projects && (
                        <div className="text-green-600 flex items-center">
                          <span className="mr-1">✓</span> Covers {company.number_of_projects} projects
                        </div>
                      )}
                      {plan.numberproject < company.number_of_projects && (
                        <div className="text-red-600 flex items-center">
                          <span className="mr-1">✗</span> Only {plan.numberproject} projects
                        </div>
                      )}
