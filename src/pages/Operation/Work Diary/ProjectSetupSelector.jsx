...
const ProjectSetupSelector = ({
  ...
  const handleSelect = (setup) => {
    const project = projects.find(
      (p) =>
        p.id === setup.project_id &&
        p.type?.toLowerCase() === setup.project_type?.toLowerCase(),
    );
  ...
  const filteredSetups = setups.filter((s) => {
    const project = projects.find(
      (p) =>
        p.id === s.project_id &&
        p.type?.toLowerCase() === s.project_type?.toLowerCase(),
    );
    const projectName = (
  ...
  return (
    <div className="relative mb-6">
      <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
        <FaProjectDiagram className="text-green-700" /> Select Project Setup
      </label>
        ...
        <input
          ...
          value={
            ...
            (currentSetup
              ? projects.find(
                (p) =>
                  p.id === currentSetup.project_id &&
                  p.type?.toLowerCase() ===
                  currentSetup.project_type?.toLowerCase(),
              )?.name || ""
              : "")
          }
          onChange={(e) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-gray-500">
                  ID: {setup.id} • Start: {setup.start_date} • Type:{" "}
                  {setup.project_type}
                </div>
              </div>
            );
