...
import { FaArrowLeft, FaArrowRight, FaBuilding, FaCalendarAlt, FaCheckCircle, FaHome, FaSearch, FaPlus, FaTrash, FaExclamationTriangle } from "react-icons/fa";
...
const OpenClawModelSetupWizard = () => {
  ...
  const [stages, setStages] = useState([
    { id: `stage-${Date.now()}`, name: "Stage 1", description: "", start: "", end: "", status: "planned", subStages: [] }
  ]);
  ...
  const handleProjectChange = async (e) => {
    ...
    const project = projectsList.find((item) => item.uid === selectedUid) || null;
    ...
    if (typeof currentUnits === 'string') {
      try { currentUnits = JSON.parse(currentUnits); } catch (err) { currentUnits = []; }
    }
    ...
    try {
      ...
      const existingSetup = allSetups.find((setup) => setup.project_id === project.id && setup.project_type?.toLowerCase() === project.type?.toLowerCase());

      if (existingSetup) {
        setExistingSetupId(existingSetup.id);
        setContractorId(existingSetup.contractor_id || "");
        const contractor = (contractorsList || []).find((c) => c.id === existingSetup.contractor_id);
        setContractorSearchTerm(contractor ? contractor.name || contractor.contractor_name : "");
        ...
        if (typeof loadedStages === 'string') {
          try { loadedStages = JSON.parse(loadedStages); } catch (err) { loadedStages = []; }
        }
        setStages(Array.isArray(loadedStages) && loadedStages.length > 0 ? loadedStages : [
          { id: `stage-${Date.now()}`, name: "Stage 1", description: "", start: "", end: "", status: "planned", subStages: [] }
        ]);
      } else {
        setExistingSetupId(null);
  ...
  const filteredContractors = (contractorsList || []).filter((contractor) =>
    (contractor.name || contractor.contractor_name || "").toLowerCase().includes(contractorSearchTerm.toLowerCase()) ||
    (contractor.phone || "").toLowerCase().includes(contractorSearchTerm.toLowerCase())
  );

  const handleContractorSelect = (contractor) => {
    setContractorId(contractor.id);
    setContractorSearchTerm(contractor.name || contractor.contractor_name || "");
  ...
  const addStage = () => {
    setStages((prev) => [
      ...prev,
      { id: `stage-${Date.now()}-${Math.floor(Math.random() * 1000)}`, name: `Stage ${prev.length + 1}`, description: "", start: "", end: "", status: "planned", subStages: [] }
    ]);
  };
  ...
  const updateStage = (id, field, value) => {
    setStages((prev) => prev.map((stage) => stage.id === id ? { ...stage, [field]: value } : stage));
  };
  ...
  const addSubStage = (stageId) => {
    setStages((prev) => prev.map((stage) => {
      ...
      return {
        ...stage,
        subStages: [...(stage.subStages || []), { id: `sub-${Date.now()}-${Math.floor(Math.random() * 1000)}`, name: "Sub-stage", description: "", status: "planned" }]
      };
    }));
  };
  ...
  const removeSubStage = (stageId, subId) => {
    setStages((prev) => prev.map((stage) => {
      if (stage.id !== stageId) return stage;
      return { ...stage, subStages: (stage.subStages || []).filter((sub) => sub.id !== subId) };
    }));
  };
  ...
  const updateSubStage = (stageId, subId, field, value) => {
    setStages((prev) => prev.map((stage) => {
      ...
      return {
        ...stage,
        subStages: (stage.subStages || []).map((sub) => sub.id === subId ? { ...sub, [field]: value } : sub)
      };
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const project = projectsList.find((item) => item.uid === selectedProjectUid);
      if (!project) {
      ...
      let res;
      if (existingSetupId) {
        res = await operationApi.updateProjectSetup(existingSetupId, requestData);
      ...
      if (res?.data) {
        if (typeof res.data.stages === 'string') {
          try { res.data.stages = JSON.parse(res.data.stages); } catch (err) { }
        }
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">OpenClaw Model Setup Wizard</h1>
            <p className="text-sm text-slate-600 mt-2">Step through project setup, upload unit plans, define stages, and submit in a guided flow.</p>
          </div>
          <div className="flex items-center gap-2 text-xs text-slate-500 uppercase tracking-[0.2em]">
            ...
            <span className="px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm">{activeStep} / 4</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {[
            ...
            >
              <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Step {item.step}</div>
        ...
        <div className="rounded-3xl bg-white border border-slate-200 shadow-sm p-6">
          {errorMessage && (
            <div className="mb-4 rounded-xl bg-rose-50 border border-rose-200 p-4 text-rose-700">{errorMessage}</div>
          )}
          {statusMessage && (
            <div className="mb-4 rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-emerald-700">{statusMessage}</div>
          )}
                  ...
                  >
                    ...
                    {projectsList.map((project) => (
                      <option key={project.uid} value={project.uid}>
                        [{project.type}] {project.name || project.project_name} {project.location ? `• ${project.location}` : ''}
                      </option>
                    ))}
                  </select>
                </div>
                    ...
                    <div className="absolute z-30 w-full mt-2 rounded-2xl border border-slate-200 bg-white shadow-lg max-h-56 overflow-y-auto">
                      {filteredContractors.length > 0 ? (
                        filteredContractors.map((contractor) => (
                          ...
                          >
                            <div className="font-semibold">{contractor.name || contractor.contractor_name}</div>
                            <div className="text-xs text-slate-500">{contractor.phone || 'No phone'}</div>
                          </button>
                        ))
                      ) : (
                        <div className="p-4 text-sm text-slate-500">No contractors found</div>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                  ...
                  <div className="mt-3 grid gap-4 sm:grid-cols-3">
                    <div className="rounded-2xl bg-white p-4 border border-slate-200">
                      ...
                      <div className="mt-2 font-semibold text-slate-900">{selectedProject.name || selectedProject.project_name}</div>
                    </div>
                    <div className="rounded-2xl bg-white p-4 border border-slate-200">
                      <div className="text-xs uppercase tracking-[0.2em] text-slate-400">Type</div>
                      <div className="mt-2 font-semibold text-slate-900">{selectedProject.type}</div>
                    </div>
                    <div className="rounded-2xl bg-white p-4 border border-slate-200">
                      <div className="text-xs uppercase tracking-[0.2em] text-slate-400">Location</div>
                      <div className="mt-2 font-semibold text-slate-900">{selectedProject.location || selectedProject.site_location || 'Not available'}</div>
                    </div>
                  </div>
                </div>
              )}
          ...
          {activeStep === 2 && (
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-semibold text-slate-900">Unit Plans</h2>
              </div>
              {units.length === 0 ? (
                ...
                <>
                  ...
                  <div className="space-y-4">
                    {filteredUnits.map((unit) => (
                      <div key={unit.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                          <div>
                            <div className="font-semibold text-slate-900">{unit.name || `${unit.type} ${unit.id}`}</div>
                            <div className="text-sm text-slate-500">{unit.size ? `${unit.size} • ` : ''}Floor {unit.floor || 'N/A'}</div>
                          </div>
                          <div className="text-sm font-medium text-slate-600">{plans[unit.id]?.length || 0} file(s)</div>
                        </div>
                        ...
                        {plans[unit.id]?.length > 0 && (
                          <div className="mt-4 space-y-3">
                            {plans[unit.id].map((file, index) => (
                              <div key={index} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3">
                                <div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                <div className="flex items-center justify-between gap-4">
                  <p className="text-sm text-slate-500">Build the execution stages for your model setup.</p>
                  <button
                    type="button"
                    onClick={addStage}
                  ...
                  <div key={stage.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex items-center justify-between">
                        <div className="text-sm font-medium text-slate-700">Sub-stages</div>
                        <button
                          type="button"
                      ...
                      <div className="mt-4 space-y-3">
                        {(stage.subStages || []).map((sub) => (
                          <div key={sub.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:flex-row md:items-center md:justify-between">
                            <input
                              type="text"
                              value={sub.name}
                        ...
                        {(!stage.subStages || stage.subStages.length === 0) && (
                          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-100 p-4 text-sm text-slate-500">No sub-stages added yet.</div>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          {activeStep === 4 && (
            <div className="space-y-6">
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 space-y-5">
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Project</h3>
                  <p className="mt-2 text-slate-900">{selectedProject?.name || selectedProject?.project_name || 'Not selected'}</p>
                  <p className="text-sm text-slate-500">{selectedProject?.type} • {selectedProject?.location || selectedProject?.site_location || 'No location'}</p>
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="rounded-2xl bg-white p-4 border border-slate-200">
                    <div className="text-xs uppercase tracking-[0.2em] text-slate-400">Contractor</div>
                    <div className="mt-2 font-semibold text-slate-900">{contractorSearchTerm || 'Not selected'}</div>
                  </div>
                  <div className="rounded-2xl bg-white p-4 border border-slate-200">
                    <div className="text-xs uppercase tracking-[0.2em] text-slate-400">Timeline</div>
                    <div className="mt-2 font-semibold text-slate-900">{dates.start || '—'} to {dates.end || '—'}</div>
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold text-slate-900">Stages</div>
                    <div className="text-sm text-slate-500">{stages.length} total</div>
                  </div>
                  <div className="mt-4 space-y-3">
                    {stages.map((stage) => (
                      <div key={stage.id} className="rounded-2xl border border-slate-200 p-4 bg-slate-50">
                        <div className="font-semibold text-slate-900">{stage.name}</div>
                        <div className="text-sm text-slate-500">{stage.description || 'No description added.'}</div>
                        <div className="mt-2 text-xs uppercase tracking-[0.18em] text-slate-400">{stage.start || 'Start not set'} → {stage.end || 'End not set'}</div>
                        {(stage.subStages || []).length > 0 && (
                          <div className="mt-3 space-y-2">
                            {stage.subStages.map((sub) => (
                              <div key={sub.id} className="rounded-2xl bg-white p-3 border border-slate-200">
                                <div className="font-medium text-slate-900">{sub.name}</div>
                                <div className="text-xs text-slate-500">{sub.description || 'Sub-stage details not provided.'}</div>
                              </div>
                            ))}
                          </div>
                        )}
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-500">Uploads are kept in the wizard session only and are not posted to the backend.</p>
                  <div className="mt-4 grid gap-3">
                    {Object.keys(plans).length === 0 ? (
                      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-100 p-4 text-sm text-slate-500">No files uploaded.</div>
                    ) : (
                      Object.entries(plans).flatMap(([unitId, files]) => (
                        files.map((file, index) => (
                          <div key={`${unitId}-${index}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 flex items-center justify-between gap-3">
                            <div>
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6">
                <div className="text-sm text-slate-500">Once you submit, the OpenClaw model setup will be saved in project setups and available across work diary modules.</div>
              </div>
            </div>
          )}
