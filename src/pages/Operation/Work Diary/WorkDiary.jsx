  ...
  return (
    <div className="p-6 bg-gray-100  min-h-screen">
        ...
        {selectedProject && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-100 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-800">
                {projectSetup?.start_date} to {projectSetup?.end_date}
              </p>
            </div>
          </div>
        )}
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
        ...
        {activeTab === "labour" && (
          <LabourDetails projectSetup={projectSetup} />
        )}
        {activeTab === "tools" && (
          <ToolsAndPlants projectSetup={projectSetup} />
        )}
        {activeTab === "stages" && <StagePassing projectSetup={projectSetup} />}
        {activeTab === "workprogress" && (
          <DailyWorkReport projectSetup={projectSetup} />
        )}
        {activeTab === "rawmaterial" && (
          <RawMaterial projectSetup={projectSetup} />
        )}
      </div>
    </div>
  );
