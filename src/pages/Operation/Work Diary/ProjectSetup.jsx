...
const ProjectSetup = ({ onProjectSelect }) => {
  ...
  const handleProjectChange = async (e) => {
    ...
    try {
      ...
      const existingSetup = setups.find(
        (s) =>
          s.project_id === selectedProj.id &&
          s.project_type?.toLowerCase() === selectedProj.type?.toLowerCase(),
      );
      if (existingSetup) {
        ...
        if (onProjectSelect)
          onProjectSelect(selectedProj, {
            ...existingSetup,
            stages: formattedStages,
            units_data: currentUnits,
          });
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (typeof updatedSetup.stages === "string")
        updatedSetup.stages = JSON.parse(updatedSetup.stages);
      if (typeof updatedSetup.units_data === "string")
        updatedSetup.units_data = JSON.parse(updatedSetup.units_data);
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="text-center">
        ...
        {selectedProject && (
          <p className="text-sm text-gray-500 mt-1">
            Selected: {selectedProject.name} • {selectedProject.location} •{" "}
            ...
            {filteredUnits.map((unit) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    className={`px-2 py-1 rounded text-xs font-medium ${plans[unit.id]?.length
      ...
      <div className="bg-gray-50 dark:bg-gray-700/50 p-6 rounded-xl border border-gray-200 dark:border-gray-600 mt-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="erp-root flex items-center gap-3">
            <button
              onClick={addStage}
