          ...
          <table className="w-full text-sm text-left text-gray-600 border border-gray-200 rounded-lg overflow-hidden">
            ...
            <tbody>
              {budgets.map((budget, index) => (
                ...
                >
                  <td className="px-6 py-4 border-r border-gray-200 font-medium text-gray-900">
                    {budget.id}
                  </td>
                  ...
                  <td className="px-6 py-4 border-r border-gray-200">
                    {budget.document_subject}
                  </td>
                  <td className="px-6 py-4 border-r border-gray-200">
                    {budget.document_date
                      ? new Date(budget.document_date).toLocaleDateString()
                      : "N/A"}
                  </td>
                  <td className="px-6 py-4 border-r border-gray-200 font-medium">
                    ₹{Number(budget.total_construction_budget).toLocaleString()}
                  </td>
                  <td className="px-6 py-4 border-r border-gray-200 font-medium">
                    ₹{Number(budget.total_saleable_budget).toLocaleString()}
                  </td>
                  <td className="px-6 py-4 border-r border-gray-200 font-medium text-green-600">
                    ₹{Number(budget.profit).toLocaleString()} (
                    {budget.profit_ratio}%)
                  </td>
