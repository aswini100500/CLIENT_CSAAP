...
const IndentEntryO = () => {
  ...
  const saveCategory = async () => {
    ...
    try {
      ...
      const created = newCats.find(c => String(c.name).toLowerCase() === String(newCategory.name).toLowerCase());
      if (created && (created.id || created._id)) {
        setProductForm(prev => ({ ...prev, category: created.id || created._id }));
      }
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      const created = newProducts.find(p => String(p.name).toLowerCase() === String(newProduct.name).toLowerCase());
      if (created && (created.id || created._id)) {
        setProductForm(prev => ({ ...prev, product: created.id || created._id }));
      }
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button type="button" onClick={() => {
                  ...
                  setNewCategory({ name: '', description: '' });
                }} className="text-green-600 p-1 rounded hover:bg-gray-100" title={showCategoryModal ? "Cancel Add Category" : "Add Category"}>
                  {showCategoryModal ? (
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button type="button" onClick={() => {
                  ...
                  setNewProduct({ name: '', category_id: '', description: '' });
                }} className="text-green-600 p-1 rounded hover:bg-gray-100" title={showProductModal ? "Cancel Add Product" : "Add Product"} disabled={!productForm.category}>
                  {showProductModal ? (
                    <svg className="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
            ...
            {loading ? (
              ...
              filteredHistory.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600">{item.indent_number}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{new Date(item.indent_date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{item.supplier_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{item.total_items}</td>
              ...
              <tr>
                <td colSpan="6" className="px-6 py-10 text-center text-gray-500">
                  {searchTerm ? 'No indents matching your search found.' : 'No indent history found.'}
                </td>
              </tr>
            )}
          </tbody>
