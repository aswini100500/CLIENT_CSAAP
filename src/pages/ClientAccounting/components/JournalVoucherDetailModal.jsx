...
const JournalVoucherDetailModal = ({
  voucherData,
  onClose,
  onEdit,
  onDownload,
  ...
  const transactions = voucherData.transactions || voucher.transactions || [];
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="jv-modal-title"
    >
      <div
      ...
      >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5 font-medium">
                  General accounting debit and credit adjustments
                </p>
              </div>
            </div>
            <button
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-[#f8faf8] border border-[#e2f2e9] text-slate-700 font-medium">
              ...
              <span className="font-extrabold text-[#042f2e]">
                {voucher.voucherNo || voucher.voucherno || voucher.id}
              </span>
            </span>
            <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-[#f8faf8] border border-[#e2f2e9] text-slate-700 font-medium">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          <p className="text-[11px] font-extrabold uppercase tracking-widest text-[#475569]">
            Journal Particulars & Line Items
          </p>
            ...
            <table className="w-full text-sm border-collapse bg-white">
              <thead className="bg-[#f0fdf4]/50 border-b border-[#e2f2e9]">
                <tr className="text-left text-slate-700">
                  ...
                  <th className="py-2.5 px-4 border-r border-[#e2f2e9] text-[11px] font-extrabold uppercase tracking-widest text-[#475569] text-right w-36">
                    Debit (₹)
                  </th>
                  <th className="py-2.5 px-4 text-[11px] font-extrabold uppercase tracking-widest text-[#475569] text-right w-36">
                    Credit (₹)
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#e2f2e9]">
                {transactions.map((t, i) => {
                  const hasDebit = Number(t.debit) > 0;
                  const hasCredit = Number(t.credit) > 0;
                  return (
                    <tr
                      key={i}
                    ...
                    >
                      ...
                      <td className="py-3 px-4 border-r border-[#e2f2e9]">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-[#042f2e] text-[13px] leading-tight">
                            {t.particulars ||
                              t.ledgerName ||
                              t.ledger?.name ||
                              "N/A"}
                      ...
                      <td className="py-3 px-4 border-r border-[#e2f2e9] text-right font-bold text-rose-600 text-[13px] tabular-nums">
                        {hasDebit ? formatAmount(t.debit) : "—"}
                      </td>
                      <td className="py-3 px-4 text-right font-bold text-emerald-600 text-[13px] tabular-nums">
                        {hasCredit ? formatAmount(t.credit) : "—"}
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr className="bg-[#f0fdf4]/40 border-t border-[#e2f2e9]">
                  ...
                  <td className="py-3 px-4 text-right font-extrabold text-rose-600 text-sm tabular-nums border-r border-[#e2f2e9]">
                    ₹ {formatAmount(totalDebit)}
                  </td>
                  <td className="py-3 px-4 text-right font-extrabold text-emerald-600 text-sm tabular-nums">
            ...
            <p className="text-xs/relaxed text-[#042f2e]  font-medium">
              {voucher.narration ? (
                ...
                <span className="italic text-slate-500">
                  No narration provided for this journal voucher.
                </span>
              )}
            </p>
          </div>
        ...
        <div className="px-6 py-4 border-t border-[#e2f2e9] flex items-center justify-between gap-3 bg-white">
          <span className="text-xs px-3 py-1 rounded-full bg-[#f0fdf4] text-[#00a651] border border-[#c6f1d6] font-bold">
            {transactions.length} entry line
          ...
          <div className="flex items-center gap-2">
            {onPrint && (
              <button
                ...
                className="inline-flex items-center gap-1.5 text-xs px-3.5 py-2 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 transition-all font-semibold cursor-pointer active:scale-[0.98]"
              >
...
export default JournalVoucherDetailModal;
