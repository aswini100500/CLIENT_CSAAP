...
const Dashboard = () => {
  ...
  const getFinancialHealth = (netProfit, totalIncome) => {
    if (totalIncome === 0) return { status: 'Neutral', color: 'text-yellow-600', bg: 'bg-yellow-100' };
    
    const profitMargin = (netProfit / totalIncome) * 100;
    
    if (profitMargin > 20) return { status: 'Excellent', color: 'text-green-600', bg: 'bg-green-100' };
    if (profitMargin > 10) return { status: 'Good', color: 'text-emerald-600', bg: 'bg-emerald-100' };
    if (profitMargin > 0) return { status: 'Fair', color: 'text-blue-600', bg: 'bg-blue-100' };
    if (profitMargin === 0) return { status: 'Breakeven', color: 'text-yellow-600', bg: 'bg-yellow-100' };
    return { status: 'Loss', color: 'text-red-600', bg: 'bg-red-100' };
  };

  const getLiquidityStatus = (assets, liabilities) => {
    if (liabilities === 0) return { status: 'Excellent', color: 'text-green-600', bg: 'bg-green-100' };
    
    const ratio = assets / liabilities;
    
    if (ratio >= 2) return { status: 'Excellent', color: 'text-green-600', bg: 'bg-green-100' };
    if (ratio >= 1.5) return { status: 'Good', color: 'text-emerald-600', bg: 'bg-emerald-100' };
    if (ratio >= 1) return { status: 'Adequate', color: 'text-blue-600', bg: 'bg-blue-100' };
    if (ratio >= 0.5) return { status: 'Poor', color: 'text-yellow-600', bg: 'bg-yellow-100' };
        ...
        <div className="text-center max-w-md mx-auto p-8">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mb-6">
              Please select an existing company or create a new one to view the dashboard and access financial data.
            </p>
          </div>
          <div className="space-y-3">
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-gray-700">
                <strong>Tip:</strong> Use the company selector in the navigation bar to choose a company or create a new one.
              </p>
            </div>
          </div>
        </div>
    ...
    <div id="dashboard-content" className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500">Overview of your company's financial health</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              getFinancialHealth(profitLossData?.totals?.net || 0, profitLossData?.totals?.totalIncome || 0).bg
            } ${
              getFinancialHealth(profitLossData?.totals?.net || 0, profitLossData?.totals?.totalIncome || 0).color
            }`}>
              {getFinancialHealth(profitLossData?.totals?.net || 0, profitLossData?.totals?.totalIncome || 0).status}
            </span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center p-4 bg-linear-to-r from-green-100 to-emerald-100 rounded-lg border border-green-200">
              ...
              <span className={`text-xl font-bold ${
                (profitLossData?.totals?.net || 0) >= 0 ? 'text-green-700' : 'text-red-700'
              }`}>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-gray-700">Income Breakdown</span>
              ...
              {showIncomeDetails && profitLossData?.income && profitLossData.income.length > 0 && (
                <div className="p-4 border-t border-gray-200">
                  <div className="space-y-3">
                    {profitLossData.income.map((item, index) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                        <div>
                          <p className="font-medium text-gray-800">{item.ledgerName}</p>
                          <p className="text-xs text-gray-500">{item.groupName}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-green-700">{formatCurrency(item.credit)}</p>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-gray-700">Expense Breakdown</span>
              ...
              {showExpenseDetails && profitLossData?.expenses && profitLossData.expenses.length > 0 && (
                <div className="p-4 border-t border-gray-200">
                  <div className="space-y-3">
                    {profitLossData.expenses.map((item, index) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-red-700">{formatCurrency(item.debit)}</p>
                          <p className="text-xs text-gray-500">
                            Opening: {formatCurrency(item.openingBalance)}
                          </p>
                        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              getLiquidityStatus(balanceSheetData?.totals?.totalAssets || 0, balanceSheetData?.totals?.totalLiabilities || 0).bg
            } ${
              getLiquidityStatus(balanceSheetData?.totals?.totalAssets || 0, balanceSheetData?.totals?.totalLiabilities || 0).color
            }`}>
              {getLiquidityStatus(balanceSheetData?.totals?.totalAssets || 0, balanceSheetData?.totals?.totalLiabilities || 0).status}
            </span>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-amber-50 rounded-lg">
                ...
                <p className="text-xl font-bold text-amber-700">
                  {formatCurrency(balanceSheetData?.totals?.totalLiabilities || 0)}
                </p>
              </div>
            </div>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-gray-700">Assets Breakdown</span>
              ...
              {showAssetsDetails && balanceSheetData?.assets && balanceSheetData.assets.length > 0 && (
                <div className="p-4 border-t border-gray-200">
                  <div className="space-y-3">
                      ...
                      <div key={index} className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-blue-700">
                            {formatCurrency(item.closingDebit - item.closingCredit)}
                          </p>
                          <p className="text-xs text-gray-500">
                            Opening: {formatCurrency(item.openingDebit - item.openingCredit)}
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-gray-700">Liabilities & Capital</span>
                  <span className="text-xs bg-amber-100 text-amber-800 px-2 py-1 rounded-full">
                    {(balanceSheetData?.liabilities?.length || 0) + (balanceSheetData?.capital?.length || 0)} items
                  </span>
                </div>
              ...
              {showLiabilitiesDetails && (
                <div className="p-4 border-t border-gray-200">
                      ...
                      {balanceSheetData.liabilities.map((item, index) => (
                        <div key={index} className="flex justify-between items-center p-3 bg-amber-50 rounded-lg">
                          ...
                          <p className="font-bold text-amber-700">
                            {formatCurrency(item.closingCredit - item.closingDebit)}
                          </p>
                        </div>
                      ))}
                    </div>
                  ...
                  {balanceSheetData?.capital && balanceSheetData.capital.length > 0 && (
                    <div className="space-y-3 mt-4">
                      <h4 className="font-medium text-gray-700 mb-2">Capital</h4>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-gray-800">Financial Health Insights</h2>
          </div>
        </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className={`font-medium ${
                  (profitLossData?.totals?.net || 0) >= 0 ? 'text-green-600' : 'text-red-600'
                }`}>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-blue-600">
                  {balanceSheetData?.totals?.totalLiabilities ? 
                    (balanceSheetData.totals.totalAssets / balanceSheetData.totals.totalLiabilities).toFixed(2) : 
                    'N/A'
                  }
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-amber-600">
                  {balanceSheetData?.totals?.totalLiabilities ? 
                    ((balanceSheetData.totals.totalLiabilities / 
                    (balanceSheetData.totals.totalAssets - balanceSheetData.totals.totalLiabilities)) || 0).toFixed(2) : 
                    '0'
                  }
                </span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-green-600">
                  {calculatePercentageChange(profitLossData?.totals?.totalIncome || 0, 0).toFixed(1)}%
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Expense Growth</span>
                <span className="font-medium text-red-600">
                  {calculatePercentageChange(profitLossData?.totals?.totalExpenses || 0, 0).toFixed(1)}%
                </span>
              </div>
            </div>
          </div>
