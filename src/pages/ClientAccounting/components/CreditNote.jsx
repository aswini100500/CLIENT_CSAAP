...
const CreditNote = () => {
  ...
  const openStockModal = (itemName) => {
    const stateToSave = {
      voucherNo, date, partyLedger, salesLedger, narration,
      gstType, gstRate, igstRate, cgstRate, sgstRate,
      partyDetails, dispatchDetails, items
    };
    sessionStorage.setItem("creditNoteState", JSON.stringify(stateToSave));
    navigate(`/accounting/client/stockItemCreation?redirect=/accounting/client/creditnote&name=${encodeURIComponent(itemName || "")}`);
  };
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (savedState) {
          ...
          if (res.data.success) {
            ...
            setSalesLedger(n.SalesLedger || n.salesLedger || n.PurchaseLedger || "");
    ...
    if (companyId && !id) {
      axios.get(`${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/voucher-util/next/${companyId}/credit_note`)
  ...
  const handleCreateCreditNote = async () => {
    ...
    const payload = {
      voucherNo, date, partyLedger, purchaseLedger: salesLedger, partyDetails, dispatchDetails, narration,
    ...
    try {
      if (isEditMode) {
        await axios.put(`${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/notes/updateCredit/${id}`, payload);
        Swal.fire("Success", "Credit Note Updated Successfully", "success");
        navigate("/accounting/client/creditNotesList");
      } else {
        const res = await axios.post(`${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/notes/createCreditNote/${companyId}`, payload);
        Swal.fire({
          title: "Saved",
  ...
  return (
    <div className="bg-[#f7f7f5] min-h-screen p-6 md:p-10 pb-20">
      ...
      <div className="flex items-end justify-between mb-7 pb-5 border-b-[1.5px] border-[#e2e2dc]">
        <h1 className="font-serif text-[30px] text-[#0f1117] m-0 leading-[1.15]">Credit Note</h1>
      </div>
      ...
      <div className="bg-white border border-[#e2e2dc] rounded-[10px] shadow-sm p-6 mb-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input className="w-full px-3 py-2 border-[1.5px] border-[#e2e2dc] rounded-md text-[14px] text-[#0f1117] outline-none focus:border-[#1a56db] transition-all" value={voucherNo} onChange={e => setVoucherNo(e.target.value)} placeholder="CN-001" />
          </div>
          <div className="flex flex-col gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-[12px] font-medium text-[#5c6070]">Party (Customer) Ledger</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[12px] font-medium text-[#5c6070]">Sales Ledger</label>
            <select className="w-full px-3 py-2 border-[1.5px] border-[#e2e2dc] rounded-md text-[14px] text-[#0f1117] outline-none focus:border-[#1a56db] transition-all" value={salesLedger} onChange={e => setSalesLedger(e.target.value)}>
      ...
      <div className="bg-white border border-[#e2e2dc] rounded-[10px] shadow-sm p-6 mb-5">
        ...
        <button className="bg-[#0d7448] text-white px-7 py-2.5 text-[14.5px] font-semibold rounded-md mt-6 hover:opacity-88 transition-opacity" onClick={handleCreateCreditNote}>Save Credit Note</button>
      </div>
