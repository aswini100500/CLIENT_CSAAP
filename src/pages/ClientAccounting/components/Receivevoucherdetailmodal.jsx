...
import { X, Hash, Calendar, ArrowDownToLine, Download, Edit } from "lucide-react";

const ReceiveVoucherDetailModal = ({ voucher, onClose, onEdit, onDownload, ledgerMap = {} }) => {
    ...
    const receiptMode = ledgerMap[voucher.receiptAccountId] || voucher.receiptAccountId || "—";
                ...
                <div className="px-6 py-5 border-b border-gray-100 shrink-0">
                    <div className="flex items-start justify-between gap-4 mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 id="rv-modal-title" className="text-base font-semibold text-gray-900 leading-tight">
                                    Receipt Voucher Details
                                </h2>
                                <p className="text-xs text-gray-400 mt-0.5">Incoming payment record</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="flex flex-wrap gap-2">
                        <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-gray-50 border border-gray-100 text-gray-600">
                            ...
                            Voucher no. <span className="font-semibold text-gray-800">{voucher.voucherId || voucher.id}</span>
                        </span>
                        <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-gray-50 border border-gray-100 text-gray-600">
                            <Calendar size={11} className="text-gray-400" />
                            <span className="font-semibold text-gray-800">{formatDate(voucher.date)}</span>
                        </span>
                    </div>
                </div>
                ...
                <div className="px-6 py-5 flex flex-col gap-5 overflow-y-auto">
                    ...
                    <div className="rounded-xl border border-gray-100 overflow-hidden">
                        ...
                        <div className="px-4 py-4 flex items-center justify-between">
                            <div>
                                <p className="text-xs text-gray-500 mb-0.5">Total amount received</p>
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden">
                            <table className="w-full text-sm">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        <th className="px-4 py-2.5 text-left font-medium text-gray-500 text-xs uppercase tracking-wider">Received From (Ledger)</th>
                                        <th className="px-4 py-2.5 text-right font-medium text-gray-500 text-xs uppercase tracking-wider">Amount (₹)</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {voucher.items.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50">
                                            <td className="px-4 py-3 text-gray-800">{ledgerMap[item.ledgerId] || item.ledgerId}</td>
                                ...
                                <tfoot className="bg-gray-50 border-t border-gray-200 font-semibold">
                                    <tr>
                                        <td className="px-4 py-3 text-right text-gray-600 text-xs uppercase tracking-wider">Grand Total</td>
                                        <td className="px-4 py-3 text-right text-blue-700 tabular-nums">₹ {formatAmount(totalAmount)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <div className="flex gap-3 bg-indigo-50 rounded-xl px-4 py-3 border border-indigo-100">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-indigo-700 leading-relaxed">
                                {voucher.narration || (
                                    <span className="italic text-indigo-400">No narration provided.</span>
                                )}
                            </p>
                        </div>
                    </div>
