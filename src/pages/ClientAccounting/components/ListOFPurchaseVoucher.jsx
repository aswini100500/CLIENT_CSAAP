...
import PurchaseVoucherDetailModal from "./PurchaseVoucherDetailModal";
...
const ListOfPurchaseVoucher = () => {
  ...
  const fetchVouchers = async () => {
    try {
      ...
      const mapped = rawData.map((v) => ({
        ...
        supplierName: v.customer || "Walk-in Supplier",
        amount: Number(v.grand_total || v.grandTotal || v.totalAmount || 0),
  ...
  const filteredVouchers = vouchers.filter((v) => {
    ...
    const voucherNoStr = (v.voucherNumber || v.invoiceNo || v.id || "")
      .toString()
      .toLowerCase();
    const supplierStr = (v.supplierName || v.customer || "")
      .toString()
      .toLowerCase();
    const narrationStr = (v.narration || "").toString().toLowerCase();
    const amountStr = (v.amount || "").toString().toLowerCase();

    const hasMatchingItem = v.items?.some((i) => {
      const itemName = (i.item || i.itemName || "").toString().toLowerCase();
      const hsn = (i.hsn_code || "").toString().toLowerCase();
      return itemName.includes(query) || hsn.includes(query);
    });
  ...
  const totalAmount = sortedVouchers.reduce(
  ...
  const generatePDF = (shouldPrint = false) => {
    ...
    doc.text("Purchase Voucher Report", 14, 26);
  ...
  const handleExportExcel = () => {
    ...
    sortedVouchers.forEach((v) => {
      const items = v.items?.length ? v.items : [{}];
      ...
      items.forEach((item, index) => {
        exportData.push({
          ...
          "Item Name": item.itemName || item.item || "-",
          "HSN Code": item.hsn_code || "-",
          Qty: Number(item.qty || 0),
          Rate: Number(item.rate || 0),
          "Line Amount": Number(item.amount || 0),
          "Voucher Total": Number(v.amount || 0),
  ...
  const handleEdit = (id) => {
    const editPath =
      loggedInRole === "employee"
        ? `/employee/hr/accounting/client/purchasevoucher/${id}`
        : `/accounting/client/purchasevoucher/${id}`;
    navigate(editPath);
  };
  ...
  const handleViewDetails = async (v) => {
    try {
      ...
      setSelectedVoucher(res.data || v);
    } catch (err) {
      console.error("Error fetching single purchase voucher:", err);
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          ...
          text: "Purchase voucher has been deleted.",
          timer: 1800,
        ...
        Swal.fire("Error", "Failed to delete purchase voucher.", "error");
      }
    }
  };
  ...
  const handleDownloadPDF = (v) => {
    ...
    const url = `${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/purchase-voucher/download/${v.id}`;
    fetch(url)
      .then((response) => {
      ...
      .then((blob) => {
        ...
        link.href = blobUrl;
        link.download = `Purchase_Voucher_${v.voucherNumber || v.id}.pdf`;
      ...
      .catch((err) => {
        ...
        if (v.pdf_path) {
          window.open(
            `${import.meta.env.VITE_ACCOUNTING_URL}/${v.pdf_path}`,
            "_blank",
          );
        } else {
          Swal.fire("Error", "Could not download PDF file.", "error");
        }
      });
  };
  ...
  const createPath =
    loggedInRole === "employee"
      ? "/employee/hr/accounting/client/purchasevoucher"
      : "/accounting/client/purchasevoucher";
                ...
                {sortedVouchers.map((v, idx) => {
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 border-r border-[#e2f2e9] text-slate-700 font-semibold text-[13px]">
                        {v.supplierName || "-"}
                      </td>
        ...
        {!loading && filteredVouchers.length === 0 && (
          <div className="px-4 py-12 text-center">
            ...
            <p className="text-[13px] mt-1 text-slate-500">
              Try adjusting your search criteria or create a new purchase
              voucher.
            </p>
          </div>
        )}
