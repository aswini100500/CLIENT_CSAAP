...
import { FileSpreadsheet, Printer, Search, RefreshCw, Layers3, UserRound } from 'lucide-react';
...
const GSTSummary = () => {
  ...
  const getSessionEmployeeId = () => {
    try {
      const userStr = sessionStorage.getItem('employeeUser') || sessionStorage.getItem('adminUser') || sessionStorage.getItem('user');
      const sessionUser = userStr ? JSON.parse(userStr) : null;
      return sessionUser?.employee_id || sessionUser?.id || sessionUser?.employeeProfileId || user?.employee_id || user?.id || null;
    } catch (e) {
  ...
  const handleExportPDF = (isPrint = false) => {
    ...
    autoTable(doc, {
      startY: 28,
      head: [['S.No', 'Date', 'Voucher Type', 'Voucher Number', 'IGST', 'CGST', 'SGST']],
      body: tableData,
      styles: { fontSize: 10 },
  ...
  const filteredData = gstData.filter((item) => {
    ...
    if (isEmployee && currentEmployeeId) {
      if (item.creator_employee_id != currentEmployeeId && item.created_by_employee_id != currentEmployeeId && item.created_by_user_id != currentEmployeeId) {
        matchesRole = false;
      }
    } else if (!isEmployee) {
      const isCreatedByEmployee = item.creator_employee_id || item.created_by_employee_id;
    ...
    const matchesSearch = (item.voucherNumber?.toString() || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-semibold text-gray-800">GST Summary</h1>
                <p className="text-xs text-gray-400 mt-0.5">Sales and Purchase Vouchers GST</p>
              </div>
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <span className="text-sm font-medium text-gray-700">GST Records</span>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">Voucher Type</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">Voucher Number</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-right">IGST</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-right">CGST</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-right">SGST</th>
                  </tr>
                </thead>
                      ...
                      <tr key={index} className="hover:bg-purple-50/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-md ${
                            item.voucherType === 'Sales' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                          }`}>
                            {item.voucherType}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm text-right text-gray-600">₹{item.igst || 0}</td>
                        <td className="px-4 py-3 text-sm text-right text-gray-600">₹{item.cgst || 0}</td>
                        <td className="px-4 py-3 text-sm text-right text-gray-600">₹{item.sgst || 0}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="6" className="px-4 py-12 text-center">
                        <Layers3 className="mx-auto mb-2 text-gray-300" size={32} />
                        <p className="text-sm text-gray-400 font-medium">No GST records found</p>
                      </td>
                    </tr>
                  )}
                </tbody>
