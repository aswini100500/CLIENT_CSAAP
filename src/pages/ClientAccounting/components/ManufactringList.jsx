...
const ManufacturingList = () => {
  ...
  const handlePrint = async () => {
    ...
    const totalValue = journals.reduce(
    ...
    doc.text(`Total Journals: ${journals.length}`, 14, 34);
    ...
    const tableData = journals.map((j, i) => [
      i + 1,
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Deleted!",
          "Manufacturing journal has been deleted.",
          "success",
        );
        fetchJournals();
      } catch (error) {
  ...
  const filteredJournals = journals.filter((j) => {
    if (isEmployeeDashboard) {
      ...
      const isCreatedByEmployee =
        j.employee_id && j.role?.toLowerCase() === "employee";
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#fcfcfb] border-b border-[#e2e2dc]">
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#5c6070]">
                  Date
                </th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#5c6070]">
                  Voucher No
                ...
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#5c6070] text-right">
                  Total Cost
                </th>
                {showEmployeeActivity && (
                  <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#5c6070] text-left">
                    Employee Name
                  </th>
                )}
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#5c6070] text-center">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#f0f0ed]">
              {filteredJournals.length === 0 ? (
                <tr>
                  <td
                    colSpan={showEmployeeActivity ? "8" : "7"}
                    className="px-6 py-10 text-center text-[#5c6070] italic"
                  >
                ...
                filteredJournals.map((j) => (
                  <React.Fragment key={j.id}>
                    <tr
                      className={`hover:bg-[#fcfcfb] transition-colors ${expandedId === j.id ? "bg-[#fcfcfb]" : ""}`}
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() =>
                              navigate(`${basePath}/manfacturing/${j.id}`)
                            }
                    ...
                    {expandedId === j.id && (
                      <tr className="bg-[#fcfcfb]">
                        <td
                          colSpan={showEmployeeActivity ? "8" : "7"}
                          className="px-8 py-6 border-b border-[#e2e2dc]"
                        >
                              ...
                              <div className="bg-white border border-[#e2e2dc] rounded-lg overflow-hidden shadow-sm text-[13px]">
                                <table className="w-full">
                                  <thead className="bg-[#f7f7f5] border-b border-[#e2e2dc]">
                                    <tr>
                                      <th className="px-3 py-2 font-semibold text-[#5c6070]">
                                        Item
                                      </th>
                                      <th className="px-3 py-2 font-semibold text-[#5c6070]">
                                        Godown
                                      </th>
                                      <th className="px-3 py-2 text-right font-semibold text-[#5c6070]">
                                        Qty
                                      </th>
                                      <th className="px-3 py-2 text-right font-semibold text-[#5c6070]">
                                        Rate
                                  ...
                                  <tbody className="divide-y divide-[#f0f0ed]">
                                    {j.components?.map((c, idx) => (
                                      <tr key={idx}>
                                        <td className="px-3 py-2 text-[#0f1117]">
                                          {c.itemName}
                                        </td>
                                        <td className="px-3 py-2 text-[#5c6070]">
                                          {c.godown}
                                        </td>
                                        <td className="px-3 py-2 text-right text-[#0f1117]">
                                          {c.qty}
                                        </td>
                                        <td className="px-3 py-2 text-right text-[#5c6070]">
                                          ₹ {Number(c.rate || 0).toFixed(2)}
                                        </td>
                                        <td className="px-3 py-2 text-right font-medium text-[#0f1117]">
                          ...
                          {j.narration && (
                            <div className="mt-6 pt-4 border-t border-[#e2e2dc]">
                              <p className="text-[11px] font-bold uppercase tracking-widest text-[#5c6070] mb-1">
                                Narration
                              </p>
                              <p className="text-[13px] text-[#0f1117] italic">
                                {j.narration}
                              </p>
                            </div>
                          )}
