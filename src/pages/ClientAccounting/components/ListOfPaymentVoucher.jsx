...
const ListOfPaymentVoucher = () => {
  ...
  const fetchPaymentVouchers = async () => {
    try {
      ...
      console.error("Failed to fetch payment vouchers", err);
      setVouchers([]);
    } finally {
      setLoading(false);
    }
  ...
  const filteredVouchers = vouchers.filter((v) => {
    ...
    const paymentModeStr = (
      v.accountTypeName ||
      ledgerMap[v.accountType] ||
      v.accountType ||
      ""
    )
      .toString()
      .toLowerCase();
    const amountStr = (v.amount || v.totalCredit || 0).toString().toLowerCase();

    const hasMatchingItem = v.items?.some((item) => {
      const ledgerName = (item.ledgerName || ledgerMap[item.ledgerId] || "")
        .toString()
        .toLowerCase();
      const itemAmt = (item.amount || "").toString().toLowerCase();
      return ledgerName.includes(query) || itemAmt.includes(query);
    });

    return (
      narrationStr.includes(query) ||
      voucherNoStr.includes(query) ||
      paymentModeStr.includes(query) ||
  ...
  const totalAmount = sortedVouchers.reduce((acc, v) => {
    const amt = parseFloat(v.amount || v.totalCredit || 0);
    return acc + amt;
  }, 0);
  ...
  const generatePDF = (shouldPrint = false) => {
    ...
    doc.text("Payment Voucher Report", 14, 26);
    ...
    const tableData = sortedVouchers.map((v, i) => [
      ...
      formatDate(v.date),
      v.voucherNo || v.id || "-",
      v.accountTypeName || ledgerMap[v.accountType] || v.accountType || "-",
    ...
    autoTable(doc, {
      startY: summaryY + 8,
      head: [
        ["#", "Date", "Voucher No.", "Payment Mode", "Amount", "Narration"],
      ],
      body: tableData,
  ...
  const handleExportExcel = () => {
    ...
    sortedVouchers.forEach((v) => {
      exportData.push({
        ...
        "Payment Mode":
          v.accountTypeName || ledgerMap[v.accountType] || v.accountType || "-",
        Amount: Number(v.amount || v.totalCredit || 0),
  ...
  const handleEdit = (id) => {
    const editPath =
      loggedInRole === "employee"
        ? `/employee/hr/accounting/client/paymentVoucher/${id}`
        : `/accounting/client/paymentVoucher/${id}`;
    navigate(editPath);
  };
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          ...
          text: "Payment voucher has been deleted.",
          timer: 1800,
        ...
        Swal.fire("Error", "Failed to delete payment voucher.", "error");
      }
    }
  };
  ...
  const handleDownloadPDF = (v) => {
    ...
    fetch(url)
      ...
      .then((blob) => {
        ...
        link.href = blobUrl;
        link.download = `Payment_Voucher_${v.voucherNo || v.id}.pdf`;
  ...
  const createPath =
    loggedInRole === "employee"
      ? "/employee/hr/accounting/client/paymentVoucher"
      : "/accounting/client/paymentVoucher";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 border-r border-[#e2f2e9] text-right font-bold text-[#042f2e] text-[13px] whitespace-nowrap">
                        {Number(v.amount || v.totalCredit || 0).toLocaleString(
                          "en-IN",
                          {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
        ...
        {!loading && filteredVouchers.length === 0 && (
          <div className="px-4 py-12 text-center">
            ...
            <p className="text-[13px] mt-1 text-slate-500">
              Try adjusting your search criteria or create a new payment
              voucher.
            </p>
          </div>
        )}
