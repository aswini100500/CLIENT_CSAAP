...
const API = `${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/notes/getCreditnotes`;
...
const CreditNoteList = () => {
  ...
  const totalAmount = sortedCreditNotes.reduce(
  ...
  const handleExportPDF = () => {
    ...
    autoTable(doc, {
      startY: tableStartY,
      head: [["#", "Date", "Voucher No.", "Party Ledger", "Amount", "Status"]],
      body: sortedCreditNotes.map((n, index) => [
        index + 1,
        formatDate(n.date),
        n.voucherNo || n.id || "-",
        n.PartyLedger || n.partyLedgerName || "-",
  ...
  const handleExportExcel = () => {
    ...
    const companyNameForExport =
      companyDetails?.name || companyName || "Company";
    const companyAddress = getCompanyAddress(companyDetails);
    const exportData = sortedCreditNotes.map((n, i) => ({
      "S.No": i + 1,
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        Swal.fire({
          ...
          text: "Credit Note has been removed.",
          timer: 1800,
  ...
  const handleEdit = (id) => {
    const editPath =
      loggedInRole === "employee"
        ? `/employee/hr/accounting/client/creditNote/${id}`
        : `/accounting/client/creditNote/${id}`;
    navigate(editPath);
  };

  const createPath =
    loggedInRole === "employee"
      ? "/employee/hr/accounting/client/creditNote"
      : "/accounting/client/creditNote";
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse bg-white">
              ...
              <tbody className="divide-y divide-[#e2f2e9]">
                {sortedCreditNotes.map((note, index) => (
                  <tr
                    key={note.id}
        ...
        {!loading && filteredCreditNotes.length === 0 && (
          <div className="text-center py-16 px-4">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 mb-4">
              {searchQuery
                ? `No credit notes match "${searchQuery}".`
