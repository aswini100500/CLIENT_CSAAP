...
import { Calendar, Search, FileText, Download, Trash2, ArrowLeft, Eye, Edit, FileDown, FileSpreadsheet, Printer, X, Package, MapPin, Truck, UserRound } from "lucide-react";
...
const ListOfSaleVoucher = () => {
  ...
  const handleView = async (id) => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/sale-voucher/single/${id}`);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this sales voucher?")) return;
    try {
      await axios.delete(`${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/sale-voucher/${id}`);
  ...
  const handleBulkImport = async (data) => {
    try {
      const grouped = {};
      data.forEach((row) => {
        const vNo = row["Voucher No"] || row["voucherNo"] || `SAL-${Date.now()}`;
        if (!grouped[vNo]) {
          grouped[vNo] = {
            date: row["Date"] || row["date"] || new Date().toISOString().split('T')[0],
            customer: row["Customer Name"] || row["customer"] || "Walk-in Customer",
      ...
      const vouchersPayload = Object.values(grouped).map(v => {
        const ledgerObj = ledgersList.find(l => String(l.name).trim().toLowerCase() === String(v.customer).trim().toLowerCase());
      ...
      const missingLedgers = vouchersPayload.filter(v => !v.ledgerId).map(v => v.customer);
        ...
        if (result.isConfirmed) {
          try {
            ...
            vouchersPayload.forEach(v => {
              if (!v.ledgerId) {
                const l = newLedgers.find(led => String(led.name).trim().toLowerCase() === String(v.customer).trim().toLowerCase());
      ...
      await axios.post(`${import.meta.env.VITE_ACCOUNTING_URL}/api/v1/sale-voucher/bulk-create`, {
        companyId,
        vouchers: finalValid
      });
  ...
  const filteredVouchers = vouchers.filter((v) => {
    ...
    return (
      ...
      v.voucherNumber.toString().toLowerCase().includes(searchQuery.toLowerCase())
    );
  });
                ...
                <div className="bg-green-50/50 rounded-xl p-4 border border-green-100">
                  ...
                  <div className="space-y-1.5 text-sm">
                    {[
                      ...
                      ["GST Amount", `₹ ${Number(selectedVoucher.gst_amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`],
                    ].map(([label, val]) => (
                      <div key={label} className="flex justify-between">
                        <span className="text-gray-500">{label}</span>
                      ...
                      <tbody>
                        {selectedVoucher.items.map((item, i) => (
                          <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            ...
                            <td className="px-4 py-2 font-medium text-gray-800">{item.item}</td>
                            ...
                            <td className="px-4 py-2 text-gray-500">{item.per || "Nos"}</td>
                            <td className="px-4 py-2 text-right font-semibold text-gray-800">₹ {Number(item.amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="bg-orange-50/50 rounded-xl p-4 border border-orange-100">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                    {[
                      ...
                      ["Dispatch Date", selectedVoucher.dispatchDate?.split("T")[0]],
