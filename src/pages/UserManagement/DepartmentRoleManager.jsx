...
import { 
  Building2, Shield, Plus, Edit2, Trash2, 
  Loader2, Save, X, Globe
} from 'lucide-react';
...
const getRoleDepartmentId = (role) => role.department_id || role.departmentId || '';
const getRoleDepartmentName = (role) => role.department_name || role.departmentName || 'Global';
...
const DepartmentRoleManager = () => {
  ...
  const { data: deptsData, isLoading: isDeptsLoading } = useSWR(token ? [deptsApiUrl, token] : null, fetcher);
  const { data: rolesData, isLoading: isRolesLoading } = useSWR(token ? [rolesApiUrl, token] : null, fetcher);
  ...
  const handleDeptSubmit = async (e) => {
    ...
    const toastId = toast.loading(editingDeptId ? "Updating department..." : "Adding department...");

    try {
      if (editingDeptId) {
        await axios.put(`${deptsApiUrl}/${editingDeptId}`, { name: deptName }, getAuthHeaders());
  ...
  const handleDeleteDept = async (id) => {
    if (!window.confirm("WARNING: Deleting this department will also delete all roles inside it. Are you sure?")) return;
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to delete department.", { id: toastId });
    }
  };
  ...
  const handleRoleSubmit = async (e) => {
    ...
    const toastId = toast.loading(editingRoleId ? "Updating role..." : "Adding role...");

    try {
      if (editingRoleId) {
        // Backend PUT updates the role name
        await axios.put(`${rolesApiUrl}/${editingRoleId}`, { role_name: roleName }, getAuthHeaders());
        toast.success("Role updated successfully!", { id: toastId });
      } else {
        // Send null for global roles when no department is selected
        const payload = { 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white rounded-4xl shadow-sm border border-slate-100 p-6 sm:p-8">
            ...
            <form onSubmit={handleDeptSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isSubmittingDept ? <Loader2 size={16} className="animate-spin" /> : (editingDeptId ? <Save size={16} /> : <Plus size={16} />)} 
          ...
          <div className="bg-white rounded-4xl shadow-sm border border-slate-100 overflow-hidden flex-1">
            ...
            <div className="p-2">
              {isDeptsLoading ? (
                <div className="py-10 text-center"><Loader2 className="animate-spin mx-auto text-blue-600" size={24} /></div>
              ) : departments.length === 0 ? (
                <div className="py-10 text-center text-sm font-bold text-slate-400">No departments found.</div>
              ) : (
                <div className="flex flex-col gap-1">
                  {departments.map(dept => (
                    <div key={dept.id} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-xl transition-colors group border border-transparent hover:border-slate-100">
                      <span className="text-sm font-bold text-slate-700 ml-2">{dept.name}</span>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleEditDept(dept)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                          <Edit2 size={14} />
                        </button>
                        <button onClick={() => handleDeleteDept(dept.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
        ...
        <div className="flex flex-col gap-6">
            ...
            <form onSubmit={handleRoleSubmit} className="space-y-4">
              ...
              {!editingRoleId && (
                <div className="space-y-2">
                  ...
                  >
                    <option value="" className="font-bold text-amber-600">Global Role (No Department)</option>
                    {departments.map(dept => (
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isSubmittingRole ? <Loader2 size={16} className="animate-spin" /> : (editingRoleId ? <Save size={16} /> : <Plus size={16} />)} 
          ...
          <div className="bg-white rounded-4xl shadow-sm border border-slate-100 overflow-hidden flex-1">
            ...
            <div className="p-2 max-h-125 overflow-y-auto">
              {isRolesLoading ? (
                <div className="py-10 text-center"><Loader2 className="animate-spin mx-auto text-amber-500" size={24} /></div>
              ) : roles.length === 0 ? (
                <div className="py-10 text-center text-sm font-bold text-slate-400">No roles found.</div>
              ) : (
                <div className="flex flex-col gap-1">
                  {roles.map(role => {
                    const roleDeptId = getRoleDepartmentId(role);
                    const isGlobal = !roleDeptId && getRoleDepartmentName(role) === 'Global';
                    const deptName = departments.find(d => String(d.id) === String(roleDeptId))?.name || getRoleDepartmentName(role);
                    
                    return (
                    <div key={role.id} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-xl transition-colors group border border-transparent hover:border-slate-100">
                      
                      <div>
                        <p className="text-sm font-bold text-slate-800 ml-2">{getRoleName(role)}</p>
                        <p className={`text-[10px] font-black uppercase tracking-widest ml-2 mt-0.5 flex items-center gap-1 ${isGlobal ? 'text-amber-500' : 'text-slate-400'}`}>
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleEditRole(role)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit Name">
                          <Edit2 size={14} />
                        </button>
                        <button onClick={() => handleDeleteRole(role.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Role">
                          <Trash2 size={14} />
                        </button>
                      </div>
