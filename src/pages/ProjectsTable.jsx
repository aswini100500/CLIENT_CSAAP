    ...
    const fetchProjects = async () => {
      ...
      try {
        ...
        const mappedProjects = projectsData.map((item, index) => ({
          ...
          name: item.name || item.project_name || item.title || 'Unnamed Project',
          manager: item.manager || item.project_manager || item.manager_name || item.assigned_to || 'Unassigned',
          progress: parseInt(item.progress || item.completion || item.completion_percentage || 0),
          budget: item.budget || item.project_budget || item.cost || '₹0',
          status: item.status || item.project_status || item.state || 'Planning',
        }));
          ...
          <table className="min-w-full text-left border-collapse">
            <thead>
              <tr className="app-section-bar border-b border-(--border-soft)">
                ...
                <th className="px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft)">Project Manager</th>
                <th className="px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft)">Progress</th>
                <th className="px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft)">Budget</th>
                <th className="px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-widest text-(--text-soft)">Status</th>
              ...
              {projects.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center">
                    ...
                    <p className="font-medium text-[14px] text-(--text-strong)">No projects found</p>
                ...
                projects.map((project) => (
                  <tr key={project.id} className="hover:bg-(--bg-subtle)/40 transition-colors duration-200">
                    ...
                    <td className="px-4 py-3 text-[13px] font-medium text-(--text-body)">{project.manager}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[11px] font-bold text-(--text-soft) min-w-8">{Math.min(project.progress, 100)}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[13px] font-semibold text-(--text-strong)">{project.budget}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${getStatusColor(project.status)}`}>
                        {project.status}
                      </span>
                    </td>
