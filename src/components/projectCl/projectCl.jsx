  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        />
        <div
          className={`relative w-full ${size} rounded-2xl bg-white shadow-xl`}
        >
...
const LoadingSpinner = () => (
  <div className="flex flex-col items-center justify-center py-12">
    ...
    <p className="mt-3 text-[13px] font-medium text-[#475569]">
      Loading projects...
    </p>
  </div>
);
...
const ProjectFormModal = ({
  ...
  const fetchClientDetailsByProjectId = async (companyName) => {
    ...
    try {
      ...
      const matchingProject = projects.find(
        (p) => p.client_company_name === companyName,
      );

      if (matchingProject) {
        const projectDetail = await apiService.getProjectById(
          matchingProject.id,
          companyId,
        );
    ...
    if (initialData && isEditing) {
      ...
      setProject({
        ...
        start_date: initialData.project?.start_date
          ? initialData.project.start_date.split("T")[0]
          : "",
        end_date: initialData.project?.end_date
          ? initialData.project.end_date.split("T")[0]
          : "",
  ...
  const handleCompanySelect = async (e) => {
    ...
    if (cachedCompany && cachedCompany.contact_person && cachedCompany.email) {
      ...
      const clientDetails =
        await fetchClientDetailsByProjectId(selectedCompanyName);
      if (clientDetails) {
        setClient((prev) => ({
          ...prev,
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleClientChange("website", e.target.value)
                  }
                  className="w-full rounded-xl border border-[#e2f2e9] px-3 py-2 text-[13px] focus:border-[#00a651] focus:outline-none focus:ring-2 focus:ring-[rgba(0,166,81,0.16)] transition-all"
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <h3 className="mb-2 text-[16px] font-bold text-[#042f2e]">
          Delete Project
        </h3>
        <p className="mb-6 text-[13px] text-[#475569]">
          Are you sure you want to delete{" "}
          <span className="font-semibold text-[#042f2e]">{projectName}</span>?
          <br />
    ...
    >
      ...
      {detail && (
        <div className="space-y-5">
            ...
            <div className="flex items-start justify-between flex-wrap gap-2">
              <div>
                ...
                <p className="text-[11px] font-mono text-[#475569] mt-0.5">
                  Code: {detail.project.project_code}
                </p>
              </div>
              <div className="flex gap-1.5">
                <span
                  className={`inline-flex rounded-full border px-2 py-0.5 text-[11px] font-medium ${STATUS_COLORS[detail.project.status]}`}
                >
                  {detail.project.status}
                </span>
                <span
                  className={`inline-flex rounded-full border px-2 py-0.5 text-[11px] font-medium ${PRIORITY_COLORS[detail.project.priority]}`}
                >
            ...
            <div className="rounded-xl bg-[#f8faf8] border border-[#e2f2e9] p-3">
              <div className="grid grid-cols-2 gap-2 text-[13px]">
                <div>
                  ...
                  <p className="font-medium text-[#042f2e]">
          ...
          <div>
            <h4 className="text-[13px] font-bold text-[#475569] mb-2">
              Project Metrics
            </h4>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <div className="rounded-xl bg-[#f8faf8] border border-[#e2f2e9] p-3">
                ...
                <p className="text-[16px] font-bold text-[#042f2e] mt-1">
              ...
              <div className="rounded-xl bg-[#f8faf8] border border-[#e2f2e9] p-3">
                ...
                <p className="text-[16px] font-bold text-[#042f2e] mt-1">
                  {detail.project.estimated_hours?.toLocaleString() || 0}
                </p>
              </div>
              <div className="rounded-xl bg-[#f8faf8] border border-[#e2f2e9] p-3">
                <div className="flex items-center gap-1 text-[11px] text-[#94a3b8]">
              ...
              <div className="rounded-xl bg-[#f8faf8] border border-[#e2f2e9] p-3">
                ...
                <p className="text-[12px] font-medium mt-1">
                  {formatDate(detail.project.end_date)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[11px] font-medium text-[#94a3b8] mb-1">
                Environment
              </p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {detail.project.technology_stack.split(",").map((tech, idx) => (
                  <span
                    key={idx}
                    className="rounded-full bg-[#f0fdf4] border border-[#e2f2e9] px-2 py-0.5 text-[11px] font-medium text-[#042f2e]"
                  >
                    {tech.trim()}
                  </span>
                ))}
            ...
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {detail.locations.map((loc) => (
                  <div
                    key={loc.id}
                    className="rounded-xl bg-[#f8faf8] border border-[#e2f2e9] p-2"
                  >
                    <p className="text-[13px] font-medium">
                      {loc.location_name}
                    </p>
  ...
  return (
    <tr className="border-b border-[#e2f2e9] hover:bg-[#f0fdf4]/70 transition-colors duration-200">
      <td className="px-4 py-3">
        <div>
          <div className="text-[14px] font-bold text-[#042f2e]">
      ...
      <td className="px-4 py-3">
        <div>
          <div className="text-[13px] font-semibold text-[#042f2e]">
      ...
      <td className="px-4 py-3">
        <span
          className={`inline-flex rounded-full border px-2 py-0.5 text-[11px] font-medium ${STATUS_COLORS[project.status]}`}
        >
          {project.status}
        </span>
      </td>
...
const ProjectsPage = () => {
  ...
  const { projects, loading, error, refetch, companyCache } =
    ...
    return (
      <div className="min-h-screen bg-[#f8faf8] flex items-center justify-center">
        <div className="text-center p-6 bg-white rounded-2xl shadow-sm border border-[#e2f2e9] max-w-md">
          ...
          <h2 className="text-[18px] font-bold text-[#042f2e] mb-1">
            Configuration Error
          </h2>
    ...
    <div className="min-h-screen bg-[#f8faf8]">
      <div className="app-shell p-4">
        <div className="max-w-7xl mx-auto space-y-6">
          <div>
            <h1 className="app-title max-w-3xl text-[24px] font-extrabold text-[#042f2e]">
              Client Projects
            </h1>
            <p className="app-subtitle mt-1 text-[13px] text-[#475569]">
              Manage and track all your client projects in one centralized
              dashboard
            </p>
          </div>
          ...
          {showFilters && (
            <div className="bg-white rounded-xl border border-[#e2f2e9] p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="app-label block mb-1.5 text-[11px] font-bold uppercase tracking-wider text-[#475569]">
                    Status
                  </label>
                  <select
                    value={statusFilter}
                ...
                <div>
                  ...
                  <select className="w-full rounded-xl border border-[#e2f2e9] px-3 py-2 text-[13px] focus:border-[#00a651] focus:outline-none focus:ring-2 focus:ring-[rgba(0,166,81,0.16)] transition-all">
          ...
          <div className="bg-white rounded-2xl border border-[#e2f2e9] overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-[14px] font-medium text-[#042f2e] mb-1">
                  No projects found
                </p>
              ...
              <>
                <div className="overflow-x-auto">
                  <table className="min-w-full">
                    <thead className="bg-[#f8faf8]">
                      <tr>
                        <th className="px-4 py-2.5 text-left text-[11px] font-extrabold uppercase tracking-wider text-[#475569]">
                          Project
                        </th>
                        <th className="px-4 py-2.5 text-left text-[11px] font-extrabold uppercase tracking-wider text-[#475569]">
                          Client
                        ...
                        <th className="px-4 py-2.5 text-right text-[11px] font-extrabold uppercase tracking-wider text-[#475569]">
                          Actions
                        </th>
                      </tr>
                    </thead>
                ...
                <div className="flex items-center justify-between border-t border-[#e2f2e9] bg-[#f8faf8] px-4 py-3">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button className="rounded-lg bg-[#00a651] px-3 py-1 text-[12px] font-medium text-white shadow-sm">
                      1
                    </button>
                    <button className="rounded-lg border border-[#e2f2e9] bg-white px-3 py-1 text-[12px] text-[#475569] hover:bg-[#f8faf8]">
                      2
                    </button>
                    <button className="rounded-lg border border-[#e2f2e9] bg-white px-3 py-1 text-[12px] text-[#475569] hover:bg-[#f8faf8]">
                      3
