...
const CustomizeSelect = ({ onSelectType, onBack,  initialSelected = [], completionStatus = {} }) => {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 relative ">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {onBack && (
          <button onClick={onBack} className="text-emerald-600 hover:text-emerald-800 font-semibold flex items-center gap-1 transition-colors">
            <FaArrowLeft /> Back
          </button>
        )}
      </div>
        ...
        {Object.keys(typeDefinitions).map((key) => {
          ...
          return (
            <button
              key={key}
              onClick={() => toggle(key)}
              className={`text-left p-5 rounded-xl border transition-all duration-150 relative ${active ? "bg-emerald-50/50 border-emerald-400/80 shadow-sm" : "bg-white border-slate-200 hover:shadow-sm"
                }`}
            >
              <div className="flex items-center justify-between">
                ...
                {active && <FaCheckCircle className="text-emerald-600 text-xl" />}
              </div>
      ...
      <div className="flex items-center justify-between mt-6">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-wider transition-all ${selected.length === 0 ? "bg-slate-200 text-slate-400 cursor-not-allowed" : "bg-emerald-600 hover:bg-emerald-700 text-white active:scale-[0.98]"
              }`}
