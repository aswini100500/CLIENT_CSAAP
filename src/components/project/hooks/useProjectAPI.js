...
export const useProjectAPI = () => {
    ...
    async (projectData) => {
      ...
      try {
        ...
        if (projectData.priceDetails) {
          ...
          const hasAny = Object.values(pd).some(
  ...
  const updateBlock = useCallback(
    async (projectId, blockId, blockData) => {
    ...
    async (projectId, unitData) => {
      ...
      try {
        ...
        const payload = {
          ...
          furnished_status:
            unitData.propertyFeatures?.furnishedStatus || "Unfurnished",
          total_built_up_area: unitData.areaDetails?.builtUpArea || 0,
        };

        if (unitData.priceDetails?.expectedPrice)
          payload.price = unitData.priceDetails.expectedPrice;
        ...
        if (unitData.staffEngaged)
          payload.staff_engaged = unitData.staffEngaged;
        if (unitData.loanProvider)
          payload.loan_provider = unitData.loanProvider;
  ...
  const createFloorDetails = useCallback(
    async (projectId, unitId, floorData) => {
      if (!projectId || !unitId) {
  ...
  const fetchRevenuePlots = useCallback(
    async (projectId) => {
      if (!projectId) return [];
      try {
        const response = await axios.get(
          `${API_URL}/${projectId}/revenue-plots`,
          {
            params: {
              slug,
              subdomain: slug,
        ...
        console.error("Fetch revenue plots error:", err);
        return [];
      }
    },
    [getAuthHeaders, slug],
  ...
  const fetchFloorDetails = useCallback(
    async (projectId, unitId) => {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/${projectId}/units/${unitId}/floor-details`,
          {
            params: {
              slug,
              subdomain: slug,
        ...
        return response.data?.data?.[0] || response.data?.data || null;
      } catch (err) {
