...
export const MainInfoSection = ({
  ...
  useEffect(() => {
    if (selectedUnit) {
      ...
      setShowCustomPossession(
        status !== "" && !standardStatuses.includes(status),
      );
      setAvailableFromMonth(
        selectedUnit.transaction_type?.availableFrom?.month || "",
      );
      setAvailableFromYear(
        selectedUnit.transaction_type?.availableFrom?.year || "",
      );
      ...
      const validApprovalStatus =
        Array.isArray(selectedUnit.approval_status) &&
        selectedUnit.approval_status.length > 0
    ...
    if (isEditing) {
      return (
        <div className="flex items-center gap-1.5">
          ...
          <button
            onClick={handleSave}
            className="text-emerald-600 hover:bg-emerald-50 p-1 rounded transition-all"
          >
            <FaCheck size={10} />
          </button>
          <button
    ...
    if (selectedUnit) {
      const updatedUnits = units.map((u) =>
        u.id === selectedUnit.id
          ? { ...u, approval_status: updatedApprovals }
          : u,
      );
      setUnits(updatedUnits);
      ...
      const updatedUnits = units.map((u) => ({
        ...u,
        approval_status: updatedApprovals,
      }));
      setUnits(updatedUnits);
    }
  };
  ...
  const generateUnits = async () => {
    ...
    try {
      const newUnits = [];
      const currentMaxId =
        units.length > 0 ? Math.max(...units.map((u) => u.id)) : 0;
      ...
      for (let i = 0; i < numUnits; i++) {
        ...
        const unitData = {
          ...
          approval_status:
            ...
            (approvalStatus[0].authority || approvalStatus[0].status)
              ? approvalStatus
              : [{ authority: "", status: "" }],
          transaction_type: {
            possessionStatus: possessionStatus,
            availableFrom: {
              month: availableFromMonth,
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <div className="flex items-center gap-1.5">
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${hasFloorDetails(selectedUnit) ? "bg-emerald-500" : "bg-slate-300"}`}
                    />
                  ...
                  <div className="flex items-center gap-1.5">
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${selectedUnit.isComplete ? "bg-emerald-500" : "bg-slate-300"}`}
                    />
                    <span className="text-xs font-medium text-slate-600">
                      Unit Core
                    </span>
            ...
            <button
              onClick={() => {
                const updatedUnits = units.map((u) =>
                  u.id === selectedUnit.id
                    ? { ...u, isComplete: !u.isComplete }
                    : u,
                );
                setUnits(updatedUnits);
              }}
              className={`mt-3 md:mt-0 px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider border transition-all ${
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider border transition-all ${
                    mainInfo.individualBoundary === true
                ...
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider border transition-all ${
                    mainInfo.individualBoundary === false
                ...
                {isEditMode && units.length > 0 ? (
                  <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      Click on units below to edit them individually
                    </p>
                  </div>
                ) : (
                  <button
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {FACILITIES.map((facility) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg border text-xs font-bold uppercase tracking-wider text-center transition-all ${
              ...
              {units.length === 0 ? (
                ...
                units.map((unit) => (
                  <div
                    ...
                    className={`p-2 rounded-xl border cursor-pointer transition-all group ${
